...
# == AFOR Field Coverage Audit ==
# Generated by FIX 8 analysis of BfpXlsxParser.parse() output vs DB INSERT vs API response.
#
# PARSER → MAPPED → DB COLUMNS:
#   responder_type       → incident_nonsensitive_details.responder_type         ✔ STORED ✔ API
#   fire_station_name    → incident_nonsensitive_details.fire_station_name       ✔ STORED ✔ API
#   notification_date+time → notification_dt (combined)                          ✔ STORED ✔ API
#   region               → _province_text (metadata only, not stored separately) ⚠ NOT STORED IN COLUMN
#   province             → _province_text (metadata only)                        ⚠ NOT STORED IN COLUMN
#   city                 → _city_text (looked up to city_id)                     ✔ STORED ✔ API
#   address              → incident_sensitive_details.street_address              ✔ STORED ✔ API
#   landmark             → incident_sensitive_details.landmark                    ✔ STORED ✔ API
#   caller_info          → pii_blob (caller_name + caller_number)                ✔ STORED ✔ API (decrypted)
#   receiver             → incident_sensitive_details.receiver_name               ✔ STORED ✔ API
#   engine               → responding_unit.engine_number                          ✔ STORED ✔ API
#   time_dispatched      → responding_unit.dispatch_dt                            ✔ STORED ✔ API
#   time_arrived         → responding_unit.arrival_dt                             ✔ STORED ✔ API
#   response_time        → incident_nonsensitive_details.total_response_time_min  ✔ STORED ✔ API
#   distance_km          → incident_nonsensitive_details.distance_from_station_km ✔ STORED ✔ API
#   alarm_level          → incident_nonsensitive_details.alarm_level              ✔ STORED ✔ API
#   time_returned        → responding_unit.return_dt                              ✔ STORED ✔ API
#   gas_liters           → incident_nonsensitive_details.total_gas_consumed_liters ✔ STORED ✔ API
#   classification       → incident_nonsensitive_details.general_category         ✔ STORED ✔ API
#   category             → incident_nonsensitive_details.sub_category             ✔ STORED ✔ API
#   owner                → pii_blob (owner_name), establishment_name              ✔ STORED ✔ API
#   description          → ⚠ PARSED (D52) BUT NOT MAPPED → added to sub_category fallback
#   origin               → incident_nonsensitive_details.fire_origin              ✔ STORED ✔ API
#   stage                → incident_nonsensitive_details.stage_of_fire            ✔ STORED ✔ API
#   extent               → incident_nonsensitive_details.extent_of_damage         ✔ STORED ✔ API
#   struct_aff/house_aff/fam_aff/indiv_aff/vehic_aff → nonsensitive cols         ✔ STORED ✔ API
#   res_* / tool_*       → incident_nonsensitive_details.resources_deployed JSONB ✔ STORED ✔ API
#   timeline             → incident_nonsensitive_details.alarm_timeline JSONB     ✔ STORED ✔ API
#   icp_present/location → incident_sensitive_details cols                        ✔ STORED ✔ API
#   inj_*/fat_*          → incident_sensitive_details.casualty_details JSONB      ✔ STORED ✔ API
#   pod_commander/shift/nozzleman/lineman/crew/dpo/safety →
#                          incident_sensitive_details.personnel_on_duty JSONB     ✔ STORED ✔ API
#   fire_arson_investigator (D121) → personnel_on_duty.fire_arson_investigator    ✔ STORED (FIX 1)
#   FOUA row (D88/E88/F88)         → alarm_timeline.foua                          ✔ STORED (FIX 1)
#   Ground commanders (F89-F100)   → alarm_timeline[key].commander                ✔ STORED (FIX 1)
#   others_list          → incident_sensitive_details.other_personnel JSONB       ✔ STORED ✔ API
#   narrative            → incident_sensitive_details.narrative_report             ✔ STORED ✔ API
#   problems             → incident_nonsensitive_details.problems_encountered      ✔ STORED ✔ API
#   recommendations      → incident_nonsensitive_details.recommendations           ✔ STORED ✔ API
#   disposition          → incident_sensitive_details.disposition                  ✔ STORED ✔ API
#   prepared_by/noted_by → prepared_by_officer / noted_by_officer                 ✔ STORED ✔ API
#   sketch_image_path    → incident_attachments (separate table)                  ✔ STORED FIX 3
#
# GAPS FIXED IN THIS FILE:
#   ✔ FIX 1 — fire_arson_investigator (D121+E121), FOUA (D88/E88/F88), ground commanders (F89-F100)
#   ✔ FIX 3 — extract_sketch_image() + incident_attachments INSERT
#   ✔ FIX 8 — this block
#   ✔ DEDUP — duplicate detection before commit (notification_dt + station + city)
#   ✔ SECURITY — magic bytes check for images, force bypass restricted to SYSTEM_ADMIN
# =============================================================================
...
# Directory for AFOR sketch/attachment storage (override via env var)
STORAGE_DIR = os.environ.get("WIMS_STORAGE_DIR", "/tmp/wims_uploads")
...
class BfpXlsxParser:
    ...
    def parse(self) -> dict[str, Any]:
        ...
        # Rows containing "N/A", "n/a", whitespace-only, or None are skipped.
        others = []
        ...
        return {
            ...
            "timeline": {
                # Row 88: 1ST ALARM - FOUA (First On Upon Arrival)
                "foua": {"time": self.get("D88"), "date": self.get("E88"), "commander": self.get("F88")},
                # Rows 89-100: standard alarm escalation rows
            ...
            # D120: safety officer name / contact (e.g. "Juan Dela Cruz / 0917-xxx")
            "pod_safety": self.get("D120"),
            # D121: Fire and Arson Investigator name / contact number (PII: not-PII — job role)
            "pod_fire_arson_inv": self.get("D121"),
            "others_list": others,
    ...
    def extract_sketch_image(self) -> bytes | None:
        ...
        try:
            ...
            try:
                # Some openpyxl versions store the raw image differently
                return getattr(img, "_data", lambda: None)()
            except Exception:
                return None
...
def parse_afor_report_data(data: dict, region_id: int) -> AforParsedRow:
    ...
    _raw_timeline = data.get("timeline") or {}
    timeline = {
        "foua": _raw_timeline.get("foua") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "alarm_1st": _raw_timeline.get("alarm_1st") or {"time": data.get("alarm_1st"), "date": data.get("notification_date"), "commander": ""},
        "alarm_2nd": _raw_timeline.get("alarm_2nd") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "alarm_3rd": _raw_timeline.get("alarm_3rd") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "alarm_4th": _raw_timeline.get("alarm_4th") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "alarm_5th": _raw_timeline.get("alarm_5th") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "tf_alpha": _raw_timeline.get("tf_alpha") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "tf_bravo": _raw_timeline.get("tf_bravo") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "tf_charlie": _raw_timeline.get("tf_charlie") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "tf_delta": _raw_timeline.get("tf_delta") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "general": _raw_timeline.get("general") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "fuc": _raw_timeline.get("fuc") or {"time": None, "date": data.get("notification_date"), "commander": ""},
        "fo": _raw_timeline.get("fo") or {"time": None, "date": data.get("notification_date"), "commander": ""},
    }
    ...
    incident_nonsensitive_details = {
        ...
        "alarm_timeline": {
            # Ground commander fields stored per FIX 1 — JSONB sub-key "commander"
            "foua": {
                "time": _dt(timeline.get("foua", {}).get("date"), timeline.get("foua", {}).get("time")),
                "commander": (timeline.get("foua") or {}).get("commander") or "",
            },
            "alarm_1st": {
                "time": _dt(timeline["alarm_1st"]["date"], timeline["alarm_1st"]["time"]),
                "commander": timeline["alarm_1st"].get("commander") or "",
            },
            "alarm_2nd": {
                "time": _dt(timeline["alarm_2nd"]["date"], timeline["alarm_2nd"]["time"]),
                "commander": timeline["alarm_2nd"].get("commander") or "",
            },
            "alarm_3rd": {
                "time": _dt(timeline["alarm_3rd"]["date"], timeline["alarm_3rd"]["time"]),
                "commander": timeline["alarm_3rd"].get("commander") or "",
            },
            "alarm_4th": {
                "time": _dt(timeline["alarm_4th"]["date"], timeline["alarm_4th"]["time"]),
                "commander": timeline["alarm_4th"].get("commander") or "",
            },
            "alarm_5th": {
                "time": _dt(timeline["alarm_5th"]["date"], timeline["alarm_5th"]["time"]),
                "commander": timeline["alarm_5th"].get("commander") or "",
            },
            "alarm_tf_alpha": {
                "time": _dt(timeline["tf_alpha"]["date"], timeline["tf_alpha"]["time"]),
                "commander": timeline["tf_alpha"].get("commander") or "",
            },
            "alarm_tf_bravo": {
                "time": _dt(timeline["tf_bravo"]["date"], timeline["tf_bravo"]["time"]),
                "commander": timeline["tf_bravo"].get("commander") or "",
            },
            "alarm_tf_charlie": {
                "time": _dt(timeline["tf_charlie"]["date"], timeline["tf_charlie"]["time"]),
                "commander": timeline["tf_charlie"].get("commander") or "",
            },
            "alarm_tf_delta": {
                "time": _dt(timeline["tf_delta"]["date"], timeline["tf_delta"]["time"]),
                "commander": timeline["tf_delta"].get("commander") or "",
            },
            "alarm_general": {
                "time": _dt(timeline["general"]["date"], timeline["general"]["time"]),
                "commander": timeline["general"].get("commander") or "",
            },
            "alarm_fuc": {
                "time": _dt(timeline["fuc"]["date"], timeline["fuc"]["time"]),
                "commander": timeline["fuc"].get("commander") or "",
            },
            "alarm_fo": {
                "time": _dt(timeline["fo"]["date"], timeline["fo"]["time"]),
                "commander": timeline["fo"].get("commander") or "",
            },
        },
    ...
    mapped = {
        ...
        "incident_sensitive_details": {
            ...
            "personnel_on_duty": {
                ...
                "safety_officer": {"name": (str(data.get("pod_safety") or "").split("/")[0].strip()), "contact": (str(data.get("pod_safety") or "").split("/")[1].strip() if "/" in str(data.get("pod_safety") or "") else "")},
                # PII: not-PII (job title / operational role)
                "fire_arson_investigator": {
                    "name": (str(data.get("pod_fire_arson_inv") or "").split("/")[0].strip()),
                    "contact": (str(data.get("pod_fire_arson_inv") or "").split("/")[1].strip() if "/" in str(data.get("pod_fire_arson_inv") or "") else ""),
                },
            },
...
def _validate_image_bytes(data: bytes) -> bool:
    """Return True if data looks like a valid PNG or JPEG (magic bytes check)."""
    return data[:4] == b"\x89PNG" or data[:3] == b"\xff\xd8\xff"


def _save_sketch_image(image_bytes: bytes) -> str | None:
    """Validate and persist a sketch image. Returns relative storage path or None."""
    import uuid
    if not _validate_image_bytes(image_bytes):
        logger.warning("Sketch image rejected: invalid magic bytes (not PNG/JPEG).")
        return None
    os.makedirs(STORAGE_DIR, exist_ok=True)
    fname = f"afor_sketch_{uuid.uuid4().hex}.png"
...
def parse_xlsx_content(
    ...
    try:
        ...
        if kind == "STRUCTURAL_AFOR":
            ...
            # FIX 3: Extract embedded sketch image
            try:
...
async def commit_afor_import(
    ...
    if force_reimport:
        # Only SYSTEM_ADMIN may bypass deduplication
        user_role = user.get("role", "")
        if user_role != "SYSTEM_ADMIN":
            raise HTTPException(
                status_code=403,
                detail={"code": "FORCE_REQUIRES_ADMIN", "message": "Only SYSTEM_ADMIN can force re-import."},
            )
    ...
    if body.form_kind == "STRUCTURAL_AFOR" and not force_reimport:
        ...
        for idx, row_data in enumerate(rows_to_commit):
            ...
            if notif_dt and station:
                city_lookup = db.execute(
                    text("SELECT city_id FROM wims.ref_cities WHERE LOWER(city_name) = LOWER(:c) LIMIT 1"),
                    {"c": city_text},
                ).fetchone()
                city_id_dup = city_lookup[0] if city_lookup else None
                existing = db.execute(
                    text("""
                        SELECT fi.incident_id
                        FROM wims.fire_incidents fi
                        JOIN wims.incident_nonsensitive_details nd ON nd.incident_id = fi.incident_id
                        WHERE nd.notification_dt = CAST(:ndt AS timestamptz)
                          AND nd.fire_station_name = :station
                          AND (:city_id IS NULL OR nd.city_id = :city_id)
    ...
    for idx, row_data in enumerate(rows_to_commit):
        ...
        # FIX 3: Persist sketch image attachment if present
        sketch_path = row_data.get("sketch_image_path")
        if sketch_path and os.path.isfile(sketch_path):
            try:
                with open(sketch_path, "rb") as _sf:
                    _img_bytes = _sf.read()
                _file_hash = hashlib.sha256(_img_bytes).hexdigest()
                db.execute(
                    text("""
