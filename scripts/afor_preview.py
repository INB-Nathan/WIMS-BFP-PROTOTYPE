...
def _validate_input_path(path: str) -> str:
    if (
        ...
        print(
            "ERROR: Remote URLs are not accepted. Provide a local file path only.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def parse_afor_worksheet(ws: Any) -> dict[str, Any]:
    ...
    for r in range(124, 133):
        ...
        if name_str and name_str.upper() not in ("N/A", "NA", "-", "—", "N.A.", ""):
            others.append(
                {"name": name_str, "designation": str(rem).strip() if rem else ""}
            )
...
def render_markdown(data: dict[str, Any], filepath: str) -> str:
    ...
    lines.append(
        "> Generated by `scripts/afor_preview.py` — verify before uploading.\n"
    )
    ...
    flat_vals = [
        v for k, v in data.items() if k not in ("timeline", "others", "problems")
    ]
...
def main() -> None:
    if len(sys.argv) < 2:
        print(
            "Usage: python scripts/afor_preview.py path/to/afor.xlsx", file=sys.stderr
        )
        sys.exit(1)
    ...
    if ws is None:
        ws = wb.active
        print(
            "WARNING: Could not detect AFOR sheet — using active sheet.",
            file=sys.stderr,
        )
