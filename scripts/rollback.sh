...
# Tagged by GitHub Actions before each deploy as backend-image-rollback:latest.
set -e

COMPOSE_FILE="/opt/wims-bfp/src/docker-compose.yml"
ROLLBACK_IMAGE="backend-image-rollback:latest"

if ! docker image inspect "$ROLLBACK_IMAGE" > /dev/null 2>&1; then
    echo "ERROR: No rollback image found ($ROLLBACK_IMAGE)"
    echo "Run 'docker tag <good-image-id> backend-image-rollback:latest' first"
    exit 1
fi
