 ...
 * - "Browse" → set 2h cookie `wims_browse_bypass`, stay on `/`, hide modal
 *
 * No dismiss "X". The only escape is closing the tab or navigating away.
 */
...
export function IntentModal() {
  ...
  // Lazy initializer reads the cookie once on first render to avoid
  // calling setState inside an effect body.
  const [visible, setVisible] = useState(() => {
    if (typeof document === 'undefined') return false;
    const bypass = getCookie(BYPASS_COOKIE);
    // Bypass cookie hides the modal; absence shows it.
    return bypass !== '1';
  });
  ...
  const handleReport = useCallback(() => {
    // Navigate to /report — the modal will NOT be bypassed by the cookie
    // because we never set it when clicking Report.
    router.push('/report');
  }, [router]);
  ...
  return (
    <>
      {/* Injected style for responsive card stacking and skeleton animation */}
      <style>{`
      ...
      >
        ...
        >
          ...
          >
            ...
            <span className="intent-card-label" style={{ color: '#e8e8ed' }}>Browse</span>
          </button>
        </div>
        ...
        >
          <strong style={{ color: 'rgba(232,232,237,0.65)' }}>No account needed</strong> to report
        </p>
      </div>
    </div>
    </>
