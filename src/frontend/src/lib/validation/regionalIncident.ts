/**
 * Shared Zod validation schemas for regional incident operations.
 *
 ...
 * - Verification action: enum Literal
 * - Bulk approve: non-empty list of positive ints
 *
 * Use z.infer<typeof schemaName> for TypeScript types.
 */
...
/** Allowed verification actions mirroring backend VALIDATOR_ACTION_MAP. */
export const verificationActionSchema = z.object({
  action: z.enum(["accept", "accept_replace", "pending", "reject"]),
  notes: z.string().max(DESC_MAX).nullable().optional(),
  original_incident_id: z.number().int().gte(0).nullable().optional(),
...
export const bulkApproveSchema = z.object({
  incident_ids: z
    ...
    .min(1, { message: "incident_ids must be a non-empty list" }),
  notes: z.string().max(DESC_MAX).nullable().optional(),
});
