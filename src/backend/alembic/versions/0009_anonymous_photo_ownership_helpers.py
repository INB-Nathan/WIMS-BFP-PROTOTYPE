"""Harden anonymous sessions and add session-bound photo ownership helpers.

Anonymous bearer tokens are generated and returned exactly once by a narrowly
scoped SECURITY DEFINER function.  Only SHA-256 hashes are persisted.  The
application role cannot insert, update, or delete anonymous session rows
directly; helper calls derive the session owner from the bearer token.

The attach helper locks the report and the complete requested photo set and
transitions the set in one statement.  Report creation remains an application
service transaction and is not performed by a helper.
...
_ANONYMOUS_SESSION_CONSTRAINTS = """
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'anonymous_sessions_token_hash_format'
          AND conrelid = 'wims.anonymous_sessions'::regclass
    ) THEN
        ALTER TABLE wims.anonymous_sessions
            ADD CONSTRAINT anonymous_sessions_token_hash_format
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE wims.anonymous_sessions
            ADD CONSTRAINT anonymous_sessions_absolute_expiry_after_creation
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE wims.anonymous_sessions
            ADD CONSTRAINT anonymous_sessions_revoked_after_creation
...
_REPORT_PHOTO_OWNER_CHECK = """
DO $$
BEGIN
    ALTER TABLE wims.report_photos
        DROP CONSTRAINT IF EXISTS report_photos_owner_xor;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        ...
        ALTER TABLE wims.report_photos
            ADD CONSTRAINT report_photos_device_owner_attached CHECK (
                uploader_device_id IS NULL OR report_id IS NOT NULL
            );
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        ...
        ALTER TABLE wims.report_photos
            ADD CONSTRAINT report_photos_owner_xor CHECK (
...
def _harden_anonymous_sessions() -> None:
    """Add expiry/revocation state and remove direct application table writes."""
    op.execute(
        "ALTER TABLE wims.anonymous_sessions"
    ...
    op.execute("ALTER TABLE wims.anonymous_sessions ALTER COLUMN absolute_expires_at SET NOT NULL")
    op.execute(
        "ALTER TABLE wims.anonymous_sessions ADD COLUMN IF NOT EXISTS revoked_at TIMESTAMPTZ"
    )
    op.execute(_ANONYMOUS_SESSION_CONSTRAINTS)

    # Helpers are the sole app-facing session path.  Keep the RLS insert policy
    # fail-closed as defense in depth, even though table DML is revoked below.
    ...
    op.execute("REVOKE ALL ON wims.anonymous_sessions FROM wims_app")


def _bind_report_ownership() -> None:
    """Bind reports to sessions while retaining legacy anonymous rows as NULL."""
    op.execute(
...
def _bind_photo_ownership() -> None:
    """Add session ownership while retaining legacy device-attached rows."""
    op.execute(
    ...
    op.execute(_REPORT_PHOTO_OWNER_CHECK)
    op.execute(
...
def _create_helpers() -> None:
    ...
    for signature in (
        ...
        # Revoke by exact signature so future overloads cannot inherit PUBLIC
        # execute through an ambiguous function name.
        function_name = {
            "TEXT": "wims.issue_anonymous_session",
            "TEXT, UUID": "wims.authorize_anonymous_pending_photo",
            "TEXT, INTEGER, UUID[]": "wims.attach_anonymous_photos",
        }.get(signature)
        if function_name:
            op.execute(f"REVOKE ALL ON FUNCTION {function_name}({signature}) FROM PUBLIC")
    op.execute("REVOKE ALL ON FUNCTION wims.validate_anonymous_session(TEXT) FROM PUBLIC")
    op.execute("REVOKE ALL ON FUNCTION wims.revoke_anonymous_session(TEXT) FROM PUBLIC")
    op.execute("GRANT EXECUTE ON FUNCTION wims.issue_anonymous_session(TEXT) TO wims_app")
    op.execute("GRANT EXECUTE ON FUNCTION wims.validate_anonymous_session(TEXT) TO wims_app")
    op.execute("GRANT EXECUTE ON FUNCTION wims.revoke_anonymous_session(TEXT) TO wims_app")
    op.execute(
...
def upgrade() -> None:
    _harden_anonymous_sessions()
    _bind_report_ownership()
    _bind_photo_ownership()
    _create_helpers()


def downgrade() -> None:
    """Remove helper/ownership additions without restoring permissive inserts."""
    op.execute("DROP FUNCTION IF EXISTS wims.attach_anonymous_photos(TEXT, INTEGER, UUID[])")
    op.execute("DROP FUNCTION IF EXISTS wims.authorize_anonymous_pending_photo(TEXT, UUID)")
    op.execute("DROP FUNCTION IF EXISTS wims.revoke_anonymous_session(TEXT)")
    op.execute("DROP FUNCTION IF EXISTS wims.validate_anonymous_session(TEXT)")
    op.execute("DROP FUNCTION IF EXISTS wims.issue_anonymous_session(TEXT)")
    op.execute("DROP INDEX IF EXISTS idx_report_photos_anonymous_session")
    op.execute("ALTER TABLE wims.report_photos DROP CONSTRAINT IF EXISTS report_photos_owner_xor")
    op.execute(
        "ALTER TABLE wims.report_photos"
        " DROP CONSTRAINT IF EXISTS report_photos_device_owner_attached"
    )
    op.execute("ALTER TABLE wims.report_photos DROP COLUMN IF EXISTS anonymous_session_id")
    op.execute(
        "ALTER TABLE wims.report_photos ADD CONSTRAINT report_photos_owner_xor CHECK ("
        "(uploader_user_id IS NOT NULL AND uploader_device_id IS NULL) OR "
        "(uploader_user_id IS NULL AND uploader_device_id IS NOT NULL))"
    )
    op.execute(
        "ALTER TABLE wims.anonymous_sessions"
        " DROP CONSTRAINT IF EXISTS anonymous_sessions_revoked_after_creation"
    )
    op.execute(
        "ALTER TABLE wims.anonymous_sessions"
        " DROP CONSTRAINT IF EXISTS anonymous_sessions_absolute_expiry_after_creation"
    )
    op.execute(
        "ALTER TABLE wims.anonymous_sessions"
        " DROP CONSTRAINT IF EXISTS anonymous_sessions_token_hash_format"
    )
    op.execute(
        "ALTER TABLE wims.anonymous_sessions DROP COLUMN IF EXISTS revoked_at, "
