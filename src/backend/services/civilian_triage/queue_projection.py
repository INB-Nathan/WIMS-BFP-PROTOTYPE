...
def get_queue(
    ...
    # Ensure every active public signal row has a durable workflow record.
    ...
    # Isolated reports get a one-member workflow cluster instead of a null-id
    # singleton that cannot be acted on.
    db.execute(
        text("""
            WITH unclustered AS (
                SELECT cr.report_id
