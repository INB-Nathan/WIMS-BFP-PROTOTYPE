...
Post-restart health polling uses a 15-second settle delay plus 45 iterations × 2s = 90s total capacity. The extra settle delay accounts for uvicorn cold-start and SQLAlchemy lazy engine initialization after a rolling restart under load. The `/health` endpoint is served directly by nginx (line 16 of `nginx.conf`) so external uptime monitors reach the gateway even if the backend is still starting.
