...
Migration `0008` / bootstrap `87_photo_preupload_schema.sql` permits a pending row with a nullable `report_id`, requires `attached_at` to match pending versus attached state, and indexes pending owner/cleanup lookups. Migration `0009` / bootstrap `88_anonymous_photo_ownership.sql` adds hash-only, lowercase-hex bearer enforcement, idle plus absolute expiry, revocation, `citizen_reports.anonymous_session_id`, and `report_photos.anonymous_session_id`; legacy NULL reports and `uploader_device_id` rows remain compatible, but device IDs are analytics-only. Migration `0010` / bootstrap `89_anonymous_pending_photo_insert.sql` adds a fixed-search-path `SECURITY DEFINER` helper for one capability-derived anonymous pending row, owner-bound idempotency, and a one-outstanding-pending-row cap. The helper forces `report_id`, `attached_at`, `uploader_user_id`, and `uploader_device_id` NULL and derives `anonymous_session_id` inside PostgreSQL. The report submission service must set the report session owner from the validated session before calling the attach helper. `FORCE ROW LEVEL SECURITY` remains enabled: attached-row staff behavior is unchanged, registered `CIVILIAN_REPORTER` access is limited to its own pending rows, and anonymous pending access is helper-bound rather than a permissive RLS exception. No broad `TRUE` policy or `BYPASSRLS` path is permitted. The backend pre-upload route now accepts an existing capability only through the Authorization header; frontend issuance/storage and offline UI remain out of scope, but report submission atomic attach is now wired (Slice B, 2026-07-12). `POST /api/civilian/reports` sets `citizen_reports.anonymous_session_id` from the validated capability (never caller-supplied) and calls `wims.attach_anonymous_photos` inside the same transaction as the `CIVILIAN_REPORT_SUBMIT` and `PHOTO_UPLOAD_ATTACH` audits; invalid/missing capability or rejected batches raise neutral 422 and roll back the report INSERT. No new RLS policy or `BYPASSRLS` was introduced. Audit emission remains in the application transaction/service. `src/backend/services/anonymous_sessions.py` is the SQL-helper adapter for issuance, validation, revocation, and capability-bound pending insertion; `src/backend/auth.py:get_anonymous_session_capability` retains the raw bearer only in a request-local, non-repr value for the helper, while `get_anonymous_session_id` remains UUID-only compatibility plumbing. Missing or invalid capabilities receive neutral 404 behavior. Slice C (2026-07-12) added `wims.attach_registered_photos` (migration `0011` / bootstrap `90`), the registered-contributor counterpart of `attach_anonymous_photos`; identical `SECURITY DEFINER` and narrow `wims_app`-only grant contract, no new RLS policy or `BYPASSRLS`. The helper performs no audit write itself — the route/service that calls it (Slice D) must emit the `PHOTO_UPLOAD_ATTACH` audit within the same transaction. Slice D (2026-07-12) wired that registered-branch attach into `POST /api/civilian/reports`: an `optional_auth`-derived `contributor_user_id` (server-only) is set on the report, the route triages registered → anonymous-capability → neutral 422, and a deliberate `except HTTPException` handler preserves the neutral 422/404 while rolling back the transaction. No new RLS policy or `BYPASSRLS` was introduced. Slice M (2026-07-13) completes the public tracking boundary shift: `wims.report_tracking_tokens` stores only SHA-256 token hashes, validation is bound to `(report_id, token_hash)` through `wims.validate_tracking_token()`, and the public tracking route is now `GET /api/civilian/reports/{report_id}/track/{tracking_token}` with neutral `404` for missing, mismatched, expired, or revoked capabilities. The old device-ID enumeration endpoint (`GET /api/civilian/reports?device_id=`) is retired with `410 Gone`, and the frontend no longer presents report-ID lookup as a supported tracking path.

## Community Safety Hub CMS (Slice E, 2026-07-12)
Migration `0012` / bootstrap `91_community_content_schema.sql` adds the CMS content model (`wims.community_content`, `wims.community_content_version`) for the Community Safety Hub. RLS posture:
- `FORCE ROW LEVEL SECURITY` is enabled on both tables — public/anonymous/authenticated requests use a non-superuser RLS-scoped application session, and `SELECT` is limited to `lifecycle_status = 'PUBLISHED' AND (expires_at IS NULL OR expires_at > now())`; the version-table read is gated on the parent's published/non-expired state via an `EXISTS` subquery.
- All `INSERT`/`UPDATE`/`DELETE` are `SYSTEM_ADMIN`-only, using the established `wims.current_user_role() = 'SYSTEM_ADMIN'` convention (no new auth/role check, no `is_system_admin()` helper, no `BYPASSRLS`).
- The pointer table is granted to `wims_app` for `SELECT, INSERT, UPDATE, DELETE`; the version table receives only `SELECT, INSERT`, so historical versions cannot be updated/deleted. Pointer writes are RLS-gated to admins. No grant is made to `PUBLIC` (schema-level `PUBLIC` revoke in `10_rls_policies.sql`).
- Historical `community_content_version` rows are append-only: publishing/rollback move the `published_version_id` pointer on `community_content`, never mutating version rows. The version table grants/policy permit INSERT but no UPDATE/DELETE, providing database-enforced immutability. Slice F emits transaction-bound `CMS_EDIT`, sensitive `CONTENT_PUBLISH`/`CONTENT_ARCHIVE`, and best-effort `CMS_EXPIRY_SYSTEM` audits; the expiry audit is written only when at least one row is archived, so idle beats do not flood the trail. UUID content IDs are stored in `new_values` because the established audit `record_id` is INTEGER.
- The partial unique index `uq_community_content_active_urgent_banner` enforces at most one PUBLISHED urgent banner. PostgreSQL partial-index predicates require IMMUTABLE functions, so the volatile `expires_at > now()` clause is excluded from the index; expiry is enforced at read time by the Slice F service via the SQL predicate.
- **Audit/IVH immutability preserved:** migration `0012` does NOT `DROP`/`ALTER`/`REVOKE`/`UPDATE`/`DELETE` `wims.system_audit_trails` or `wims.incident_verification_history`; the existing append-only/immutability posture for those tables is unchanged by this slice. `system_audit_trails` append-only enforcement on the final partitioned schema is provided by `100_audit_trail_immutability.sql` / Alembic `0031` (parent-level triggers raising on UPDATE/DELETE, GH #732).
...
- `17_immutable_records.sql` originally created `no_delete_audit` and `no_update_audit` rules, but `72_partition_audit_trail.sql` replaces `wims.system_audit_trails` and rules cannot enforce on partitions. Final-schema append-only enforcement is now provided by `100_audit_trail_immutability.sql` (clean bootstrap) and Alembic `0031` (existing databases): parent-level BEFORE UPDATE/DELETE row triggers on `wims.system_audit_trails` raise an error (SQLSTATE 55000) on any prohibited mutation and are cloned onto current and future partitions. This covers application roles (RLS deny, zero rows) and superuser-capable maintenance paths (trigger raises). GH #732 closed the gap; regression coverage lives in `tests/integration/test_audit_trail_immutability.py`.
...
### Device-token blocklist persistence repair (2026-07-19)

`wims.device_blocklist` is the durable history for HMAC device-token hashes; it never stores the raw signed cookie value. Alembic `0027` creates the table, hash/active indexes, forced RLS, SYSTEM_ADMIN-only `device_blocklist_admin_all` policy, and `device_blocklist.repeat_offender_threshold` configuration on upgraded deployments, where bootstrap SQL alone does not run. The existing service retains the separate Redis `device:block:{hash}` hot-path check and fail-open behavior if Redis is unavailable.

`DEVICE_TOKEN_SIGNING_KEY` is a required production secret for issuing correlatable signed device tokens and is wired from the deploy host's untracked `.env.production` into the backend; no fallback key is supplied. Rows without a correlated hash cannot be device-blocked: the UI shows a disabled action with the reason, and the API independently returns 400 for a device-block request with no hash. Historical threat logs are not backfilled.
