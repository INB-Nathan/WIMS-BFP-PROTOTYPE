...
sources: [raw/frs/frs-auth.md, raw/frs/frs-complianceanddataprivacy.md, raw/frs/frs-intrusiondetectionandnetworkingmonitoring.md, raw/frs/frs-threatdetectionwithexplainableai.md, raw/codebase/codebase-snapshot-2026-05-14.md, src/keycloak/demo-otp-provider, src/keycloak/bfp-realm.json, src/keycloak/import/bfp-realm.json, src/postgres-init/82_civilian_report_photos.sql, src/postgres-init/87_photo_preupload_schema.sql, src/postgres-init/88_anonymous_photo_ownership.sql, src/backend/alembic/versions/0008_photo_preupload_schema.py, src/backend/alembic/versions/0009_anonymous_photo_ownership_helpers.py, src/backend/services/anonymous_sessions.py, src/backend/services/report_photos.py, src/backend/auth.py, src/backend/utils/exif.py]
status: draft
---
...
Migration `0008` / bootstrap `87_photo_preupload_schema.sql` permits a pending row with a nullable `report_id`, requires `attached_at` to match pending versus attached state, and indexes pending owner/cleanup lookups. Migration `0009` / bootstrap `88_anonymous_photo_ownership.sql` adds hash-only, lowercase-hex bearer enforcement, idle plus absolute expiry, revocation, `citizen_reports.anonymous_session_id`, and `report_photos.anonymous_session_id`; legacy NULL reports and `uploader_device_id` rows remain compatible, but device IDs are analytics-only. The report submission service must set the report session owner from the validated session before calling the attach helper. Fixed-search-path `SECURITY DEFINER` helpers derive ownership from the bearer, revoke direct application session-table DML, and lock/atomically attach complete same-session photo sets. `FORCE ROW LEVEL SECURITY` remains enabled: attached-row staff behavior is unchanged, registered `CIVILIAN_REPORTER` access is limited to its own pending rows, and anonymous pending access is helper-bound rather than a permissive RLS exception. No broad `TRUE` policy or `BYPASSRLS` path is permitted. Upload and UI routes remain out of scope for this schema slice; audit emission remains in the application transaction/service. `src/backend/services/anonymous_sessions.py` is the SQL-helper adapter for issuance, validation, revocation, and pending-photo authorization; `src/backend/auth.py:get_anonymous_session_id` accepts only an Authorization bearer and returns only the derived UUID, with neutral 404 for supplied invalid capabilities. The `/api/civilian/photos/upload` route remains deferred because migration 0008 still denies anonymous pending INSERT; `services.report_photos.upload_pending_photo` is an explicit fail-closed service boundary until that RLS policy and encrypted write pipeline are deployed.
