...
- **Audit/IVH immutability preserved:** migration `0012` does NOT `DROP`/`ALTER`/`REVOKE`/`UPDATE`/`DELETE` `wims.system_audit_trails` or `wims.incident_verification_history`; the existing append-only/immutability posture for those tables is unchanged by this slice. The known final-schema `system_audit_trails` UPDATE/DELETE enforcement gap remains tracked in [[gaps/frs-codebase-gap-register]] and is not widened here.
