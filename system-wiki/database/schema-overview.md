...
- Verification/immutability: `wims.incident_verification_history` has final-schema UPDATE/DELETE blocking rules. `wims.system_audit_trails` is append-only on the final partitioned schema via parent-level BEFORE UPDATE/DELETE triggers (`100_audit_trail_immutability.sql`, Alembic `0031`) that raise an error on any prohibited mutation and are cloned to current and future partitions (GH #732).
