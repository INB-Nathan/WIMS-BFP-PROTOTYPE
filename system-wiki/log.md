...
## [2026-06-21] fix | B1+B5 thermo-nuclear review: singleton policy enforcement on correction path + TOCTOU-robust member count

- **`src/backend/services/civilian_triage/workflow.py`:**
  - **B1 fix (correct_terminal_report_command):** Added cluster membership lookup and `validate_singleton_terminal_status` call after terminal-status check. Previously, a validator could REJECT a singleton report then immediately CORRECT it to ACTIONED, defeating the singleton-action policy. Now the correction path looks up the report's cluster, locks member rows with FOR UPDATE, counts members, and calls `validate_singleton_terminal_status(new_status, member_count)` before applying the update. If the report has no cluster membership, the singleton check is skipped (report is treated as having member_count=0, which also blocks ACTIONED since 0 <= 1).
  - **B5 fix (apply_terminal_action_command):** Replaced unprotected `SELECT COUNT(*) FROM wims.citizen_report_cluster_members WHERE cluster_id = :cid` with `SELECT report_id FROM wims.citizen_report_cluster_members WHERE cluster_id = :cid FOR UPDATE` + `len(fetchall())`. This locks the membership rows against concurrent merges/splits that would make the count stale (TOCTOU race).
- **Tests:** `tests/test_civilian_triage_module.py` — 13/13 pass (was 4/4). Added 9 tests:
  - 4 correction-path tests: blocks ACTIONED on singleton, allows REJECTED_DUPLICATE on singleton, allows ACTIONED on multi-member cluster, skips check when report has no cluster.
  - 1 B5 test: verifies FOR UPDATE is used for member count query and correct event flow.
  - 4 `validate_singleton_terminal_status` pure-function tests: blocks ACTIONED for count=1, blocks ACTIONED for count=0, permits ACTIONED for count=2, permits all REJECTED_* statuses for any count.
- **Validation:** `ruff check` clean, `ruff format --check` clean, 13/13 pytest pass, broader `-k triage` suite 14/14 pass.

## [2026-06-21] fix | B3 thermo-nuclear review: tests encoded the pre-singleton-cluster contract

- **Context:** the singleton-cluster commit (`408f702a`) made every active non-terminal report get a durable `CLUSTER_MONITORING` cluster, so the queue response no longer carries `cluster_id: null` for isolated reports. The cluster/singleton split is now driven by `member_count <= 1` on the frontend. Four existing tests still encoded the pre-singleton-cluster contract and would fail under the new behavior.
- **Backend test fixes (`src/backend/tests/integration/test_triage_queue.py`):**
  - `test_isolated_report_stays_as_singleton` → `test_isolated_report_gets_singleton_cluster`: now asserts the isolated report appears in exactly one queue entry whose `cluster_id` is a real int (not null) and whose `member_count == 1`. The old "appears as a cluster_id=null singleton, not in any cluster" assertion is unreachable.
  - `test_queue_materializes_cluster_only_for_related_reports` → `test_queue_materializes_cluster_for_every_active_report`: the same query now asserts that all three reports (one isolated + two related) have a durable cluster row in `wims.citizen_report_cluster_members`. The old "isolated report must have NO cluster" assertion is the exact inverse of the new contract.
- **Frontend test fixes (`src/frontend/src/app/incidents/triage/page.test.tsx`):**
  - Mock entry: cluster 2's `cluster_id: null as unknown as number` → `cluster_id: 2` to reflect the new backend contract (no more null cluster_ids for active reports).
  - `renders metrics bar with correct counts and polled time`: flipped `Clusters: 2, Individuals: 1` → `Clusters: 1, Individuals: 2` to match the `member_count > 1` cluster/singleton split.
  - `strips HTML tags from report description and follow-ups in cluster modal`: `inspectBtns[1]` → `inspectBtns[2]` (cluster 3 with HTML is now the second singleton, not the second multi-member cluster) and title assertion `Cluster 3` → `Singleton report` (the title is now member_count-driven, not cluster_id-driven).
  - `shows Inspect on singleton and opens singleton-mode modal`: updated comment to reflect the new button layout (`[0] = cluster, [1..n] = singletons`).
- **Production code touchup (`src/frontend/src/app/incidents/triage/page.tsx`, 1 line):** the modal title at line 581 used to key off `openCluster.cluster_id` (`'Cluster N'` for non-null, `'Singleton report'` for null). Now that `cluster_id` is always a real int for active reports, the `'Singleton report'` title was unreachable. Switched the predicate to `openCluster.member_count <= 1` so the singleton title is shown for any cluster with a single member, matching the queue split.
- **Tests:** `src/backend/tests/test_civilian_triage_module.py` — 13/13 pass (no change to the module-level suite — the B3 fixes only touch the integration-level test file). `src/frontend/src/app/incidents/triage/page.test.tsx` — 13/13 pass.
- **Validation:** `ruff check` clean on backend, `ruff format --check` clean, `ast.parse` clean on test files, vitest 13/13 pass.
- **Residual risks:** integration tests in `tests/integration/test_triage_queue.py` require the Docker stack to actually execute, so they are only syntax-verified here. `test_related_reports_auto_cluster` is now vacuous (it just checks that any nearby report has a cluster, which is trivially true under the new contract) — it still passes but no longer exercises the auto-clustering intent of its name.
