...
Branch: `AuditMoreGapsFix` (off `origin/master`).
...
- **Root cause 1 — `contact_number` is a custom attribute, not an identity field.** `src/backend/api/routes/user.py` was passing `contact_number=body.contact_number` to `update_user_profile` in `keycloak_admin.py`, which stored it as a Keycloak `attributes.contact_number` entry. Keycloak's user-profile validation rejects undeclared custom attributes by default and 400/500s the request — but the DB UPDATE for `wims.users.contact_number` ran *after* the Keycloak call, so it never executed. The user's change was silently dropped and the frontend saw a 502.
- **Root cause 2 — `update_user_profile` was raising 502 even for fields it owns.** `first_name`/`last_name` are stored in Keycloak only, but the call was wrapped in a `try/except` that re-raised as 502. When Keycloak was unreachable for any reason, the entire profile update (including the contact_number the user might have been trying to change) failed.
- **Backend fix (`src/backend/api/routes/user.py`):** (a) DB writes for `contact_number`, `email_opt_in`, `push_opt_in` now run **before** the Keycloak call, so a Keycloak outage cannot lose DB-backed changes. (b) `contact_number` is intentionally **not** passed to `update_user_profile` — it is an application field, the GET path already reads it from `wims.users`, and pushing it to Keycloak is what was triggering user-profile validation. (c) The Keycloak call is now best-effort: on `KeycloakError` it logs and sets `keycloak_sync_failed = True` instead of raising. (d) Return-status logic now: both failed → 502; keycloak failed only → `200 partial` "Profile saved locally; identity provider sync will retry. Your name may not be reflected until the next login."; db failed only → `200 partial` (existing message); both succeeded → `200 ok`.
- **Backend test (`src/backend/tests/test_profile_email.py`):** `test_update_profile_keycloak_failure_returns_502` renamed to `test_update_profile_keycloak_failure_returns_partial` and now asserts 200 + `status=="partial"` + message contains "identity provider". Added `test_contact_number_only_change_succeeds_without_keycloak_call` (Keycloak broken, contact_number change returns 200 ok and `update_user_profile` is never called — regression guard for root cause 1). Added `test_contact_number_change_persists_to_db` (verifies the DB UPDATE for `contact_number` is committed, with or without Keycloak). The pre-existing `TestKeycloakProfileUpdate::test_contact_number_update_merges_existing_attributes` is unchanged — it tests the `update_user_profile` *function* in isolation, which still accepts `contact_number` for other callers.
- **Validation:** `pytest tests/test_profile_email.py tests/test_auth_email_verification.py tests/test_notification_prefs.py tests/test_dashboard_widgets.py` → 64 passed. `ruff check` clean, `ruff format --check` clean. Frontend `npx vitest run src/app/profile` → 11 passed. Frontend frontend test was not changed (the response contract `{status, message}` was preserved; "partial" still renders as error toast in the existing handler at `profile/page.tsx:131`).
- **Deviation:** Existing behavior of returning 502 on Keycloak failure (asserted by `test_update_profile_keycloak_failure_returns_502`) is deliberately changed. The new contract: DB always wins; Keycloak is best-effort; 502 only when both fail. Justified because the previous behavior made a single IDP outage block every profile edit, and the "partial" response shape was already part of the public contract.
- **Wiki update:** This log entry.

## [2026-06-22] fix(dashboard): user-scope encoder widgets + show region name in account info

- **Scope:** Two independent display/scoping fixes. (1) All `REGIONAL_ENCODER` dashboard widgets were counting the *region's* incidents; they should count the *encoder's own* work. Labels updated to make the scope unambiguous. (2) `/profile` "Assigned Region" row showed the bare `assigned_region_id` integer instead of the human-readable region name.
- **Backend (`src/backend/api/routes/dashboard.py`):** `drafts`, `submitted_today`, `pending_validation`, `by_alarm_level` switched from `region_id = :rid` to `encoder_id = :uid`. `by_category` SQL gained the `{region_filter}` placeholder. The placeholder logic in the loop now resolves to `AND fi.encoder_id = :uid` for `REGIONAL_ENCODER` on `total_incidents` and `by_category`, and to empty string for `NATIONAL_VALIDATOR` / `NATIONAL_ANALYST` / `SYSTEM_ADMIN` (preserves their global view). `params["uid"]` is set whenever the caller is an encoder, even when `assigned_region_id` is missing.
- **Frontend (`src/frontend/src/components/dashboard/widget-definitions.ts`):** `Drafts` → `My Drafts`, `Submitted Today` → `My Submissions Today`, `Pending Validation` → `My Pending Validation`, `By Alarm Level` → `My Incidents by Alarm Level`. `Total Incidents` and `By Category` labels kept (shared with other roles; the encoder's data is now user-scoped, but the label remains descriptive).
- **Frontend (`src/frontend/src/app/profile/page.tsx`):** Extracted `assignedRegionDisplay` const that maps the role + id to a label: `NATIONAL_ANALYST` → "All Regions", `SYSTEM_ADMIN` → "National", otherwise `PH_REGIONS.find(...).regionName` with `Region {id}` fallback (mirrors the `afor/import/page.tsx:596` pattern). JSX now renders `{assignedRegionDisplay}`.
- **Frontend test:** `src/frontend/src/app/profile/__tests__/profile.test.tsx` "shows region ID for REGIONAL_ENCODER" test rewritten to expect `"Region X - Northern Mindanao"` (assignedRegionId=13). Pre-existing duplicate `type User` import (lines 9 + 28) removed as a side-effect unblock — it was a pre-#271 parse error that prevented the test file from running.
- **Backend test (`src/backend/tests/test_dashboard_widgets.py`):** Updated `test_encoder_count_widgets` mock prefix from `"fi.region_id = :rid"` to `"fi.encoder_id = :uid"`. Added `test_encoder_widgets_use_encoder_id_not_region_id` — captures every SQL string + params dict the dashboard endpoint emits for an encoder across all 6 encoder widgets, then asserts each (a) contains `encoder_id`, (b) does not contain `fi.region_id = :rid`, (c) carries `:uid` in its params. Regression guard for the user-scoping decision.
- **Validation:** `pytest tests/test_dashboard_widgets.py` → 13 passed (was 12). Broader backend suite → 1419 passed; 19 pre-existing integration failures (live DB required) + 137 pre-existing env errors, none new. `ruff check` clean, `ruff format --check` clean. `npx vitest run` → 996 passed (was 996; profile test rewritten, not added — net same). `npm run lint` → 0 errors.
- **Deviation:** None from the user request. Note: widget offline behavior is unchanged (still shows error banner when offline); user did not ask to fix that, so not in scope.
- **Wiki update:** This log entry. No subsystem page changes needed (regional-dashboard.md does not cover widget scope; profile fix is purely a display string).

## [2026-06-22] fix(tests): 7 CI failures stale after WS1 XFF→trusted_client_ip migration

- **Scope:** Close 7 backend CI failures (1 missing-binary, 1 stale assertion, 5 rate-limiter bucket pollution) uncovered when the WS1 XFF→`trusted_client_ip` migration landed. All four fixes are test-side; no production code changed.
- **Fix 1 — `tests/test_audit_ip_trust.py`:** Replaced `subprocess.run(["rg", ...])` (FileNotFoundError on GitHub runner) with a pure-Python `pathlib` walk + substring scan over `*.py` under `backend_root`, skipping the `tests/` subtree. Same violation filter (alias assignment, legacy definition, function definition, call-site substring) — same assertions, no external binary.
- **Fix 2 — `tests/test_privacy.py::TestConsentEndpoint`:** The 3 IP-capture tests asserted the pre-fix XFF-first behavior. The new contract from gap #14 / DS-07 is "X-Real-IP wins, XFF ignored, fallback to socket peer". Rewrote as 3 tests pinning the new contract: `test_consent_x_real_ip_is_hashed` (positive — send `X-Real-IP`, assert its hash), `test_consent_xff_is_ignored_prevents_spoofing` (regression guard — send only X-Forwarded-For, assert hash is NOT the XFF hash and IS the `testclient` fallback), `test_consent_no_ip_header_falls_back_to_socket_peer` (no headers → `testclient`). All assertions verifiable: `hash_client_ip('203.0.113.42')` = `39c197...` (the value the old test expected), `hash_client_ip('testclient')` = `8686fe...` (the value CI was returning).
- **Fix 3 — `tests/conftest.py::flush_public_rate_limit`:** Added two missing Redis namespace patterns to the autouse fixture: `wims:rl:public_consent:*` (`/api/auth/consent`, 5/IP/hr) and `wims:rl:public_notify:*` (`/api/civilian/reports/{id}/notify`, 5/IP/hr). These were added by the WS1 rate-limiter migration but the conftest namespace list was never updated. Docstring updated to enumerate all 4 namespaces and explain why `wims:rl:public_*` was added.
- **Fix 4 — `tests/test_public_abuse_controls.py`:** (a) `TestPublicAuditLog::test_consent_endpoint_logs_audit` patch path corrected from `utils.public_abuse.rate_limit_public` to `api.routes.consent.rate_limit_public` (the route binds the name into its own module via `from utils.public_abuse import rate_limit_public` — patching the source module doesn't intercept the call site). (b) `TestConsentRateLimit` rate-limit tests changed from `x-forwarded-for` to `x-real-ip` so the test's per-IP isolation matches the header `trusted_client_ip` actually reads. Notify test was already using `x-real-ip` — no change.
- **Validation:** `pytest tests/test_audit_ip_trust.py tests/test_privacy.py tests/test_public_abuse_controls.py tests/test_consent_rate_limit.py` → 48 passed, 0 failed. Broader 11-file regression sample (csrf_middleware, blocked_ip_middleware, ip_blocklist_*, audit_integrity, audit_utils) → 145 passed, 0 failed. `ruff check` clean (1 unused-import fixed). `ruff format --check` clean.
- **Deviation:** None — fixes match the security contract established by gap #14 / DS-07 (XFF is client-controlled, must be ignored).
- **Wiki update:** This log entry. gap-register: new "Test-stale-after-WS1" entry (CLOSED). architecture/pwa-tests-cicd.md: conftest namespace list + isolation rationale updated to all 4 namespaces.

## [2026-06-22] feat(security/frontend): XFF cleanup + civilian 429 specificity + #419 XAI load guard

- **Scope:** Three independent workstreams closing gaps left by PR #446 and the IP blocklist feature. (WS1) All app-layer client-IP reads migrated from `get_client_ip` (XFF-first, spoofable) to `trusted_client_ip` (X-Real-IP first, never XFF) — 16 call sites across 5 files, plus nginx defense-in-depth across all 3 configs. (WS2) Civilian 429 error now shows specific timing message via `ApiRequestError.retryAfter` instead of alarming generic "call 911" boundary — `errors.ts` extraction, `public-transport.ts` fix, `page.tsx` render. (WS3) #419 regression tests locking in no-XAI-on-page-load behavior — backend + frontend guards.
- **Backend (WS1):** `main.py` + `consent.py` rate limiters swapped (Tier 1, commit `b19b8092`). 15 audit-trace call sites in `incidents.py`, `validator.py` (4), `afor.py`, `encoder_crud.py` (9) swapped (Tier 2, commit `0158babe`). All 3 nginx configs set `X-Real-IP $realip_remote_addr` on every location block (Tier 3, commit `e303438e`). `test_nginx_forwarded_headers.py` rewritten with new parameterized assertion.
- **Backend (WS2):** `civilian.py:344` detail string includes `{_retry_minutes} minutes` (commit `b03a9e26`).
- **Frontend (WS2):** `ApiRequestError` extracted to shared `src/frontend/src/lib/api/errors.ts`; `public-transport.ts` throws with `.status` + `.retryAfter` (commit `8bd15937`). `page.tsx` renders specific timing message on 429 (commit `843e6ce7`).
- **Backend (WS3):** `test_summary_endpoint_does_not_call_xai` regression test in `test_security_monitoring.py` (commit `4311d9c2`).
- **Frontend (WS3):** no-analyze-on-mount guards for monitoring + system pages; manual-analyze-called-exactly-once test (commit `372cbf7b`).
- **Validation:** All 6 CI gates green — ruff check (0), ruff format (232 files), pytest (10 new + 1592 pre-existing pass; 19 pre-existing failures from unapplied migrations, not regression), npm run lint (0 errors), npx vitest run (87 files, 990 tests, 0 fail), next build (exit 0).
- **Deviation:** #419 bypassed #415 blocker (justified in spec — #415 needs migration 62 not applied to running DB; #419 tests lock in existing good behavior).
- **Commits (newest first):** `9f7d7840` (ruff format follow-up) + `372cbf7b` (WS3 frontend #419 tests) + `4311d9c2` (WS3 backend #419 test) + `843e6ce7` (WS2 frontend page.tsx render) + `8bd15937` (WS2 frontend errors.ts + transport) + `b03a9e26` (WS2 backend civilian 429 detail) + `0158babe` (WS1 Tier 2 audit sweep) + `e303438e` (WS1 Tier 3 nginx) + `6c22deeb` (WS1 test fix) + `b19b8092` (WS1 Tier 1 P0 rate limiters).
- **Wiki update:** This log entry. security-baseline: new section covering all 3 workstreams. gap-register: close "rate-limiter XFF bug" high-risk target; add `get_client_ip` deprecation entry.

## [2026-06-22] feat(admin/monitoring): actionable threat rows + IP blocklist with repeat-offender escalation

- **Scope:** Make `/admin/monitoring` threat-log rows actionable so the XAI narrative leads to enforcement, not just awareness. With 25,229 HIGH threats detected (1,261 pages at PAGE_SIZE=20), per-row actions alone are insufficient — bulk + filter-scoped actions handle the scale. New `wims.ip_blocklist` table + Redis native TTL keys (`ip:block:{ip}`, hot-path source of truth, zero Postgres in middleware) + `BlockedIPMiddleware` (403, fail-open) + repeat-offender escalation (3rd block → permanent) + critical-IP allowlist + self-IP guard (X-Real-IP, never XFF).
- **Backend (6 new endpoints):** `POST /api/admin/security-logs/{log_id}/block-source-ip`, `POST /api/admin/security-logs/block-by-filter` (with `?preview=true` dry-run + 500-IP hard cap), `POST /api/admin/security-logs/bulk-action` (block_ip / dismiss / false_positive), `DELETE /api/admin/security-logs/{log_id}` (soft-delete via shared `dismiss_security_log` helper), `DELETE /api/admin/ip-blocklist/{ip}`, `GET /api/admin/ip-blocklist`. New `src/backend/services/ip_blocklist.py` + `src/backend/tasks/ip_blocklist.py` (Celery beat, 5-min resync). New `BlockedIPMiddleware` in `main.py` uses Redis `EXISTS` only (no Postgres in hot path). 29 new pytest tests.
- **Frontend (`/admin/monitoring` only, `system/page.tsx` untouched):** 4 new filters (source_ip, date_from, date_to, q) ported from `/admin/system`. 4 per-row action groups: HITL 3-button verdict (Confirm Threat / False Positive / Request More Info), Block Source IP, Create Incident, Delete Alert. Bulk bar (sticky, maroon left-accent) with 3 actions on selected rows. Filter-scoped S3 "Block all IPs in current filter" with 500-IP cap preview + repeat-offender breakdown. New `BlockedIpsPanel` (3-tier row visual: amber/red/maroon accents; repeat-offender "Confirmed Attacker" badge). 56 new vitest tests.
- **Security (verified against 2 SOTA-model reviews):** X-Real-IP primary, never parse XFF (spoofable in local/CI). Redis native TTL keys (zero Postgres in middleware). Already-active no-op (no false escalation). Critical-IP allowlist (`ip_blocklist.allowlist` in `system_config`). 500-IP cap on filter-block (504 fix at 25k scale). RLS session via `get_db_with_rls`. 24h TTL default, permanent via repeat-offender or manual toggle. Soft-delete preserves forensic trail (FK violations on 34_security_incident + 52_breach_notifications).
- **Validation:** 6 CI gates green: ruff check (0), ruff format (232 files), pytest (29/29 blocklist + 1592 pre-existing pass; 19 pre-existing failures from unapplied migrations, not regression), npm run lint (0 errors), npx vitest run (87 files, 990 tests, 0 fail), next build (exit 0).
- **Migration apply:** `postgres-init/` is first-boot only (CLAUDE.md:33); `wimsbfp.tech` is already up. `65_ip_blocklist.sql` must be applied manually to the running prod DB: `docker compose exec -T postgres psql -U postgres -d wims -f /postgres-init/65_ip_blocklist.sql`. Documented in PR.
- **Commits:** 14 — `b77218b7` (migration 65) + `a2defd85` (service) + `70ddcb08` (block_by_filter + resync) + `c40617ec` (4 endpoints) + `5dd532f3` (ip-blocklist router) + `6def2857` (middleware + boot resync) + `caa20eb2` (Celery beat) + `49f05e34` (types + client) + `1e3c9410` (BlockedIpsPanel) + `4b84bf28` (filters + per-row actions) + `267772a0` (bulk + S3) + `01028f7a` (panel mount).
- **Wiki update:** This log entry. api-route-map +6 routes. security-baseline +IP blocklist section. gap-register: closes "XAI narrative not actionable" gap; FRS does not specify IP blocking (genuine product gap).
...
- **Root cause:** `src/frontend/src/lib/api/offlineAdmin.ts` `fetchAdminSecurityLogsOfflineAware` was retyped (commit `12e45818` "fix(lint): replace any[] with SecurityLog[]") from `{ items: any[]; total }` → `{ items: SecurityLog[]; total }`. `SecurityLog` (`src/types/api.ts:71`) is the wrong shape for the `/admin/security-logs` Suricata threat-log endpoint — it has `{ event_type, ip_address, user_agent, details }`, while the backend (`api/routes/admin/security.py:128-172`) actually returns `{ source_ip, destination_ip, suricata_sid, severity_level, raw_payload, xai_narrative, xai_confidence, admin_action_taken, resolved_at, reviewed_by, hitl_decision, classification, suricata_signature, suricata_category }`. The disjoint shapes made the pre-existing cast `result.response.items as ThreatLogItem[]` at `page.tsx:120` a TS2352 (neither type assignable to the other). The cast itself originated in `ba6b0b2a` (M8) and was harmless while the wrapper was loosely typed.

- **Fix (Option A, surgical):** Add `export interface SecurityThreatLog` to `src/frontend/src/types/api.ts` (after `SecurityLog`) mirroring the backend response shape and the already-correct local `interface SecurityLog` in `src/app/admin/system/page.tsx:83-99`. Retype the wrapper in `src/frontend/src/lib/api/offlineAdmin.ts` (import line 25 + return type line 73) from `SecurityLog` → `SecurityThreatLog`. `page.tsx:120` is unchanged — the cast is now a valid narrowing (`ThreatLogItem` is a subset of `SecurityThreatLog` with compatible nullability) instead of a type lie. 2 files, +21/-2.

- **Out of scope (flagged, not fixed):** `legacy.ts:116 fetchSecurityLogs()` is also typed `SecurityLog[]` for `/security-threat-logs` (no call site, no build break). The shared `SecurityLog` interface in `@/types/api` is stale/wrong for the threat-log endpoint (Option B territory). `system/page.tsx` declares its own local `SecurityLog` (shadows the import) — could later import `SecurityThreatLog` instead.

- **Validation (run locally, exact CI commands):** `cd src/frontend && npm run lint` → exit 0 (0 errors, 20 pre-existing warnings in unrelated files); `npx vitest run` → 947 PASS, 0 FAIL; `NEXT_PUBLIC_AUTH_API_URL=http://localhost:8080/auth NEXT_PUBLIC_BASE_URL=http://localhost:3000 npm run build` → exit 0 (was red with TS2352, now green, full route tree rendered). Backend gates not run (no backend files touched).

- **Wiki update:** This log entry. No `system-wiki/gaps/frs-codebase-gap-register.md` change (type bug, not an FRS gap). `system-wiki/backend/backend-infrastructure.md:162` already documents `SecurityThreatLog` as the ORM model for `wims.security_threat_logs`; the new frontend type aligns the frontend contract with that backend model.
...
- **`src/backend/tests/test_civilian_triage_module.py`:** Added a regression test that verifies `get_queue()` resets RLS context after materialization commit and before the main SELECT.
- **`system-wiki/subsystems/civilian-reporting-phase2.md`:** Documented the RLS-context reset requirement for the queue projection.
...
## [2026-06-21] fix | HITL threat decision 500 — UUID not JSON serializable (production-only regression on M8d)

- **Symptom (live VPS):** Admin → Threat Telemetry → inspect Suricata alert → Confirm Threat / False Positive returns "Server failed while applying the threat decision. The alert was not updated; please retry or check backend logs." Frontend maps any HTTP 500 from `PATCH /admin/security-logs/{id}` to this message (`app/admin/system/page.tsx:hitlErrorMessage`). "View Related Evidence" panel was also a dead end (empty list).
- **Root cause:** `src/backend/api/routes/admin/security.py` `update_security_log()` built `decision_dict = {"reviewed_by": _admin["user_id"], ...}` then called `json.dumps(decision_dict)`. In production `get_current_wims_user` returns `user_id` as a `uuid.UUID` (read straight from `wims.users.user_id`), which `json.dumps` cannot serialize → `TypeError` → unhandled → 500. This broke **every** HITL action (CONFIRM_THREAT, FALSE_POSITIVE, REQUEST_MORE_INFO) on **every** alert, 100% of the time. Live traceback confirmed: `PATCH /api/admin/security-logs/59413 HTTP/1.0 500`, `TypeError: Object of type UUID is not JSON serializable` at `security.py:284`.
- **Downstream symptom (not a separate bug):** "View Related Evidence" (`GET /admin/security-logs/{id}/related-audit`) queries `system_audit_trails` for rows tied to the alert. The only writer of such a row is a **successful** HITL decision (`log_system_audit(... "HITL_REVIEW", table_affected="security_threat_logs", record_id=log_id ...)`). Because every decision 500'd before reaching that call, no audit row was ever written → endpoint correctly returned `items: []`. For never-actioned alerts an empty list is also legitimately correct (ingestion writes no audit row).
- **Why CI missed it:** `tests/test_admin_new_routes.py` fixture `mock_admin_user()` set `user_id` to the **string** `"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11"`. Strings serialize fine, so the HITL PATCH tests went green. Production uses a real `uuid.UUID`. Mock-vs-reality type mismatch.
- **Fix:** `src/backend/api/routes/admin/security.py:284` — `"reviewed_by": str(_admin["user_id"])`. Coerces the UUID to its canonical string form, matching what the frontend and existing tests already expect. Scope check: grepped every `json.dumps(` in backend; this was the only call feeding a raw `uuid.UUID` `user_id` without `default=str` (`utils/audit.py` already uses `default=str`).
...
## [2026-06-22] feat(offline): read-caching complete for every role + storage/SW/eviction fixes (T1–T14)

- **Scope:** Final wave of the `docs/superpowers/plans/2026-06-21-offline-cache-every-role.md` plan. Completes encrypted + unencrypted offline **read**-caching across all four authenticated roles (admin / validator / analyst / reference) and fixes the storage layer, service worker, and eviction triggers. This task added the read-cache layer; the encoder offline **write** queue (`offlineOps` / `syncEngine`) and the civilian anonymous submit queue (`publicOfflineOps`) were already in place and were NOT changed here.

- **Storage layer (T1, `src/frontend/src/lib/offlineStore.ts`):** `DB_VERSION` 5 → 6, new `REFERENCE_STORE = 'reference-cache'` plaintext object store for per-user-namespaced reference data. `cacheAnalyticsResponse`/`getCachedAnalyticsResponse` → generic `cacheReadResponse`/`getReadCachedResponse` (encrypted) + new `cacheReferenceData`/`getCachedReferenceData` (plaintext, caller-supplied userId-namespaced key). Records now carry their own `ttlMs`; `evictExpiredReadCache` / `evictExpiredReferenceData` cursor-scan and prune by per-record expiry, capped at 500 deletions/pass. `setActiveOfflineUser(prev → next)` now also calls `clearReferenceDataForUser(prev)` (regex prefix sweep `^reference:{prevUserId}:`) on top of the existing crypto-key wipe so the prior user's plaintext RLS-scoped data cannot survive a shared-device logout/login (pushback P1).

- **Orchestrators (T2, `src/frontend/src/lib/api/offlineBase.ts`):** added `offlineAwareReference<T>(cacheKey, args, prefix, ttlMs, userId, fetcher, errorMessage)` — same control flow as `offlineAware` but uses the unencrypted `REFERENCE_STORE` with `buildCacheKey(\`${prefix}:${userId}\`, cacheKey, args)`. Shared result envelope `OfflineResult<T> = { response, fromCache, cachedAt? }`.

- **Domain wrappers (T4–T7):** 12 new offline-aware wrappers in 4 lib files:
  - `api/offlineAdmin.ts` (×11): `fetchSystemHealthOfflineAware`, `fetchSystemMetricsOfflineAware`, `fetchWorkerStatusOfflineAware`, `fetchActiveSessionsOfflineAware`, `fetchAuditLogsOfflineAware`, `fetchAdminSecurityLogsOfflineAware`, `fetchSecurityLogsSummaryOfflineAware`, `fetchAnomaliesOfflineAware`, `fetchBreachesOfflineAware`, `fetchAdminConfigOfflineAware`, `fetchRateLimitsOfflineAware`.
  - `api/offlineValidator.ts` (×2): `fetchOperationalMapOfflineAware`, `fetchValidatorAuditLogsOfflineAware`.
  - `api/offlineAnalytics.ts` (×1): `fetchAnalystIncidentWildlandDetailOfflineAware`.
  - `api/offlineReference.ts` (×3): `fetchRegionsOfflineAware(userId)`, `fetchProvincesOfflineAware(userId, regionId)`, `fetchCitiesOfflineAware(userId, provinceId)`.

- **API abstraction first (T3, `src/frontend/src/lib/api/validator.ts`):** `fetchOperationalMap(params)` and `fetchValidatorAuditLogs(params)` extracted from inline `apiFetch` calls in the validator pages before the offline-aware wrappers were built around them. `OperationalMapParams`, `AuditLogParams`, `AuditResponse` types exported.

- **Shared UI (T8, `src/frontend/src/components/ui/StaleCacheBanner.tsx`):** new component rendering nothing on `cachedAt == null`, otherwise `<StickyBanner tone="amber">` with `"Showing cached data — reconnect to refresh. from HH:MM:SS"` (or caller-supplied message). Mounted on every rewired page that returns `fromCache: true` from a wrapper.

- **Service worker (T9, `src/frontend/public/sw.js`):** `CACHE_NAME` bumped to `wims-bfp-cache-v12`. Three new analyst shells precached: `/dashboard/analyst/incidents/1`, `/dashboard/analyst/incidents/1/wildland`, `/dashboard/analyst/comparative`. `canonicalPath()` extended to collapse the three analyst URL families to those shells. New `PREFETCH_ROLE` `message` handler warms the SW cache for the post-login role (admin / analyst / validator / encoder) using `Promise.allSettled` so a single failed route does not abort the rest. Admin and validator routes are NOT precached — they rely on the encrypted IndexedDB read cache.

- **Post-login role prefetch wiring (T14, `src/frontend/src/context/AuthContext.tsx`):** after a successful login + role is known, `navigator.serviceWorker.controller?.postMessage({ type: 'PREFETCH_ROLE', role })` is sent (guarded for `controller === null` and SW-unsupported browsers).

- **Eviction wiring (T10):** four triggers run the per-record eviction pair (`evictExpiredReadCache` + `evictExpiredReferenceData`) — boot (via `LayoutShell` mount → `maybePruneCaches()`, 1h `wims:cachePruneAt` guard), every 25 successful cache writes (`incrementCacheWriteCount()` inside `offlineAware` / `offlineAwareReference`), after every successful sync-batch completion (`wims:sync-complete` listener), and on user switch (`setActiveOfflineUser` calls `clearReferenceDataForUser(prev)`). All four are best-effort (`try/catch` + dev-mode `console.warn`); the write counter is reset BEFORE the eviction call so a slow eviction does not re-fire on the next write.

- **Page rewires (T11–T13, 12 pages, no `useAutoSync`):** admin full rewires (5) → `/admin/monitoring`, `/admin/anomalies`, `/admin/breach`, `/admin/system/config`, `/admin/system/rate-limits`. Validator full rewires (2) → `/dashboard/validator/map`, `/dashboard/validator/audit`. Analyst wildland full rewire (1) → `/dashboard/analyst/incidents/[id]/wildland` (new `wildland.test.tsx`). Reference-data swap rewires (4) → `/dashboard` (filter dropdowns), `/dashboard/analyst` (filter dropdown), `/dashboard/analyst/[workflow]` (filter dropdown), `/admin/system` (filter dropdown). Each full rewire mounts `useNetworkStatus()` (NOT `useAutoSync`) + `StaleCacheBanner` and renders a friendly "unavailable offline" state on cache-miss errors. The 4 reference swap pages only changed the regions/provinces/cities fetches — the page bodies are unchanged.

- **Validation:** `npx vitest run` → 864/864 pass (775 pre-T1 baseline + 89 net new). `npx eslint` clean. `cd src/backend && ruff check .` → 0 issues. `cd src/backend && ruff format --check .` → 225 files already formatted. `npx tsc --noEmit` on `src/frontend` → 63 pre-existing errors in 12 files (no new errors introduced by this branch; the same errors existed on the pre-T1 baseline). **CI gate `npm run build` FAILS** with one error: `src/lib/api/offlineReference.ts(18,15) error TS2459: Module '"./legacy"' declares 'Region' locally, but it is not exported` — `Region` / `Province` / `City` are imported by `legacy.ts` from `types/api` but are NOT re-exported from `legacy.ts`; this is a pre-existing tsc error inherited from T4 (introduced when the file was added in commit `98188a3f`), not introduced or worsened by T15. A targeted fix would be `export type { Region, Province, City } from '@/types/api'` in `legacy.ts`. **Wiki commit withheld per the `docs/agents/ci-preflight.md` rule** ("If any gate fails, do NOT commit wiki changes — report the failure and stop").
...
## [2026-06-22] fix: three bugs — audit log UUID→username, anomaly ack 500, breach advance false negative

- **Bug 1 — Audit log shows UUID instead of username.** `get_audit_logs()` returned the raw `user_id` (UUID) column from `system_audit_trails`. The frontend displayed this directly. **Fix:** Added `LEFT JOIN wims.users u ON sat.user_id = u.user_id` to the audit log query; response now includes `user_name` alongside `user_id`. The frontend displays `user_name` when available, falling back to `user_id`. Files: `src/backend/api/routes/admin/audit.py` (SQL + response), `src/frontend/src/types/api.ts` (AuditLogEntry type), `src/frontend/src/app/admin/audit/page.tsx` (display).
- **Bug 2 — Breach advance shows "failed to advance status" but refresh shows updated status.** The backend PATCH endpoint commits (`db.commit()`) before the response is fully serialized. If the response delivery fails (network issue, timeout), the frontend sees an error even though the DB was updated. **Fix:** The `confirmStatusAdvance` catch block now calls `fetchBreachesOfflineAware()` to refresh local state on failure, so the UI matches the server. The error message changed from "The record has not been changed" to "Refresh to see the current state" to avoid misleading the user. File: `src/frontend/src/app/admin/breach/page.tsx`.
- **Bug 3 — Acknowledge anomaly returns 500.** `json.dumps()` in `anomalies.py` received a `uuid.UUID` object for `"status_changed_by": admin["user_id"]`. Python's `json` module cannot serialize UUIDs by default, raising `TypeError` → 500. **Fix:** Changed to `str(admin["user_id"])`. File: `src/backend/api/routes/admin/anomalies.py`.
...
- **Test gap that allowed the regression:** ownership tests in `test_civilian_api.py` use the `_insert_report_raw_bypassing_encryption` direct-DB helper, which bypasses the FastAPI route AND the encrypt-on-submit path. The encrypt-on-submit → ownership-check round-trip was never tested in CI.
...
- **TDD:** 2 tests written — both went RED on master (`404 Report not found` and `[]` empty list), both went GREEN after the 4-line fix in `_encrypt_witness_pii()`. Full backend suite: 27 tests in `test_civilian_api.py` + 30 in `test_privacy.py`, all green.
...
## 2026-06-24 — Email provider switch handoff created

- **Context:** The live-notifications work at `25d5eca` wired Gmail SMTP + FCM, but the email channel is blocked at the network layer on the DigitalOcean VPS (outbound 25/465/587 to `smtp.gmail.com` is blocked). FCM is now testable end-to-end after the device_id fix at `8427b533`.
- **Action:** Created `system-wiki/sessions/2026-06-24_email-provider-switch-handoff.md` (~20 KB) for the next session. The handoff maps the current email infrastructure (`sender.py` 141 lines, `tasks/notifications.py:165-205` Celery task with SMTP-specific retry logic, Keycloak realm SMTP at `bfp-realm.json:1530-1541`, 7 Jinja2 templates, 11 SMTP env vars), identifies the file change surface, and provides a starting-point provider landscape table (SendGrid, Mailgun, AWS SES, Postmark, Resend, Brevo, Mailjet) — all HTTP-API-based, all work where SMTP egress is blocked.
...
- **Spec:** `docs/superpowers/specs/2026-06-24-keycloak-email-theme-design.md` (v2.1, commit `4847ceb6`). Plan: `docs/superpowers/plans/2026-06-24-keycloak-email-theme.md` (v2). Closed by PR #453.
...
## 2026-06-24 — Keycloak email theme: FreeMarker auto-escape rejects `?html` (post-PR #453 fix)

- **Problem:** PR #453 (commit `c5cbc57c`) deployed the 3 HTML email templates with `${displayName?html}`, `${link?html}`, and `${msg(...)?html}` — carried over from the v2.1 spec's "XSS fix" for the FreeMarker operator-precedence bug. The first live V1 tripwire (password reset, triggered at 2026-06-24 18:40 UTC for `nathancabrales10@gmail.com`) returned Keycloak's user-facing `Failed to send email, please try again later.` `docker logs wims-keycloak` showed: `freemarker.core.ParseException: Syntax error in template "html/password-reset.ftl" in line 8, column 98: Using ?html (legacy escaping) is not allowed when auto-escaping is on with a markup output format (HTML), to avoid double-escaping mistakes`. **Root cause:** Keycloak 24 ships FreeMarker 2.3.32. Since FreeMarker 2.3.30, the default for HTML output format is `auto_esc=true` (auto-escape on). When auto-escape is on for a markup output format, the `?html` legacy escape builtin is rejected by the parser as a double-escape safeguard. The v2.1 self-review (17-row identifier-consistency table citing every claim to the Keycloak 24 source) verified identifiers but did not verify parseability of `?html` under FreeMarker 2.3.32 with auto-escape on.
- **Fix:** 7 `?html` call sites removed from 3 HTML templates (`password-reset.ftl` × 2, `email-verification.ftl` × 2, `executeActions.ftl` × 3). The `<#assign displayName = (user.firstName?has_content)?then(user.firstName, user.username)>` pattern is preserved. XSS protection now provided by FreeMarker's auto-escape (which is on for HTML output format and converts `& < > " '` to entities). The 3 text templates (`text/*.ftl`) start with `<#ftl output_format="plainText">` and have auto-escape off; they don't use `?html` and require no change. Spec patched to v2.1.1 with a post-mortem.
- **Files changed (7 total):** 3 code files (7 `?html` removals + 1 expanded comment in `password-reset.ftl`), 1 spec patch (`docs/superpowers/specs/2026-06-24-keycloak-email-theme-design.md` v2.1.1), 3 wiki files (`system-wiki/architecture/infrastructure-config.md`, `system-wiki/gaps/frs-codebase-gap-register.md`, `system-wiki/log.md`). Direct commit to `master` after PR #453 merge — small surgical fix, no PR review required per operator instruction.
