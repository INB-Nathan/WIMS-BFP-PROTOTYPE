...
- **Validation:** `cd src/backend && pytest -q tests/test_ai_service_retry.py` — 18 passed. VPS hot-swap confirmed `qwen2.5:3b` is installed and backend loaded the updated service file; a cold manual analysis completed in ~4m03s and warm analysis in ~3m10s before JSON repair was added.
