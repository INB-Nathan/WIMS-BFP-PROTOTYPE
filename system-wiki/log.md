## [2026-07-16] feat(public-surface): report wizard + receipt with routing feedback (#613)

- **Scope:** Civilian fire-reporting overhaul (Slice P2). Replaced the legacy `/report` page with a 5-step Report Wizard (Category → Location → Details → Photo → Review) that preserves the existing #604 loading/error UX. Added a Receipt view with a QR code (`qrcode.react@^4.2.0`, user-approved dependency exception, ISC) encoding the tracking URL plus the `https://wims.test/tracking?...` string, and a 3-state routing feedback panel (nearest station / head directly to station / wait) driven by the nearest-station lookup. Added `DraftManager` (`src/frontend/src/components/report-wizard/DraftManager.ts`) for in-progress draft persistence across the wizard steps.
- **Frontend-only:** reuses foundations #619 (`PublicHeader`/`LayoutShell`) and #620 (Tabler icons, `SeverityIndicator`); the wizard's nearest-station lookup reuses the already-existing `fetchNearbyStations(lat, lon)` from `src/frontend/src/lib/api/legacy.ts` (via the `@/lib/api` barrel) rather than adding a new helper. No new npm dependency beyond `qrcode.react`; no backend/SQL/RLS/auth changes; the public `/api/ref/fire-stations` endpoint (from #612) is unchanged.
- **CI collision fix (important):** the first implementation introduced a coordinate-taking `fetchStations(lat, lon)` into `src/frontend/src/lib/api/map.ts`, colliding with #612's zero-argument `fetchStations()` already in that same module. GitHub's synthetic merge with `master` produced duplicate exports (`Multiple exports with the same name "fetchStations"` at merged `map.ts:105`), breaking the Frontend transform. Fix commit `0d7cd891` restored #612's single zero-argument `fetchStations()`, kept `PublicFireMapInner` calling it, and pointed the wizard at `fetchNearbyStations(lat, lon)`. Confirmed the synthetic merge now contains exactly 1 `fetchStations` definition. Also added `NEXT_PUBLIC_AUTH_API_URL` + `NEXT_PUBLIC_BASE_URL` to `.env.example` (`48133dc0`) to satisfy `oidc.ts` fail-fast at build.
- **Review+harden (SDD):** 4-voice Gate 1 found 1 blocker (submit/loading/error UX from #604 untested) + 2 warnings → hardened with 5 tests (submit error banner + retry, validation-block guard, in-flight "Submitting…" disabled, back-nav data persistence, clipboard `execCommand` fallback). Phase F re-review: all gates PASS, no blockers. Lint 0 errors, tsc clean on changed files.
- **Tests:** `src/frontend/src/app/report/__tests__/page.test.tsx` (wizard flow, routing 3-state, draft persistence, error/retry, QR render) and `src/frontend/src/app/report-routing.test.ts` (tracking-URL routing string). 18 #613-specific tests pass; full vitest suite 133 files / 1408 tests green.
- **Wiki:** this entry appended. No FRS/code gap-register change (behavior matches IA spec §4/§5).
- **Validation:** `cd src/frontend && npx eslint <changed> && npx vitest run <changed tests> && npx tsc --noEmit && npx next build` — all green. CI run `29488673752`: Backend, Docker Build, Frontend, Security Audit, Security Scan, Validate Migrations, Merge Gate — **all 7 PASS**.
- **Merge:** PR **#625** (base `master`), open and CI-green, **awaiting user merge confirmation** (repo guardrail: parent does not merge autonomously). Commits: `80cb3f14` (feat), `80e5ccde` (routing test), `48133dc0` (.env.example), `0d7cd891` (fetchStations collision fix).
