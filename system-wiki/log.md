...
## [2026-07-17] fix(frontend+infra): report route map and controlled Metro Manila routing (#668, #552)

- **Scope:** The civilian report receipt now renders a real SSR-safe Leaflet map with report/station markers, a road polyline when `routing_geometry` is valid, and a dashed estimated line otherwise. `RouteFeedback` derives its routed state from valid geometry, not `routing_data_source`. `Wizard.tsx` adds bounded non-overlapping tracking polling (immediate request plus five 2-second retries) that stops on valid geometry, exhaustion, token absence, report replacement, or unmount. A production-only internal OSRM service processes a pinned Metro Manila dataset; backend routing rejects malformed/non-LineString OSRM geometry; and `scripts/provision-osrm-metro-manila.sh` provisions the dataset from a SHA-256-verified extract with atomic activation.
- **Frontend:** New `src/frontend/src/components/map/RouteMap.tsx` + `RouteMapInner.tsx` (domain-neutral `RouteMap`, reused by the receipt; tracking-v2 reuse is a separate planned slice). Removed the `RouteFeedback` inline SVG. Tests: `RouteMap.test.tsx`, `RouteMapWrapper.test.tsx`, `RouteFeedback.test.tsx`, and `app/report/__tests__/page.test.tsx` polling cases. `npx vitest run` focused suites 35/35 passed; `npm run lint` 0 errors (36 pre-existing warnings).
- **Backend:** `services/routing.py` validates OSRM geometry as a finite-coordinate GeoJSON LineString before returning it. Tests in `tests/test_routing.py` extended (14/14). New `tests/test_osrm_infra_config.py` asserts OSRM exists only in the production overlay, has no host port, uses a static internal address, mounts the dataset read-only, logs no coordinates/URLs, and that backend/Celery receive the internal URL without a hard health dependency (17/17 backend routing+infra tests passed).
- **Infra:** `src/docker-compose.prod.yml` adds `osrm/osrm-backend:v5.27.1` at `172.18.0.9`, internal-only, read-only dataset mount, `WARNING` verbosity, pinned resource limits; `src/docker-compose.yml` adds the `osrm:172.18.0.9` `/etc/hosts` entry (inert when routing disabled). `src/.env.production.example` documents `OSRM_DATA_DIR`. `src/osrm/metro-manila.env` pins the extract URL, SHA-256, version, and image. `scripts/provision-osrm-metro-manila.sh` + `scripts/tests/test-provision-osrm-metro-manila.sh` pass with stubbed curl/docker.
