...
| **Security Threat Logs** | Suricata/XAI threat telemetry with advanced filters & pagination (#348) | `fetchAdminSecurityLogs()` → `GET /api/admin/security-logs` | Table with source/dest IP, severity, Suricata SID, raw payload, XAI narrative/confidence; Analyze button runs `analyzeSecurityLog()`; HITL modal: decision buttons (Confirm Threat / False Positive, plus Request More Info where enabled) and Create Incident. Successful decisions keep the modal open, show a reviewed state, and retain View Related Evidence access for both reviewed and unreviewed logs. Backend response is paginated (`items`, `total`, `limit`, `offset`) and includes `hitl_decision` JSONB. **Advanced filter bar (#348):** severity chips (LOW/MEDIUM/HIGH/CRITICAL), Source IP input, Date From/To inputs; Reset All Filters button. **Pagination controls (#348):** prev/next with page indicator; 20 items/page. **Auto-reload:** triggered by filter/pagination state changes via useEffect comparison of telemetry filter key. **Error state:** inline alert shown on fetch failure; HITL 500s are surfaced with a decision-specific retry/backend-log message. |
| **System Audit Trails** | Dedicated page `/admin/audit` with full filters, pagination, offline caching (#352) | `fetchAuditLogsOfflineAware()` → `GET /api/admin/audit-logs` | The admin hub now shows a CTA card linking to `/admin/audit`; full audit table, advanced filters (q, user_id, action_type, table_affected, ip_address, date_from, date_to), expandable old/new values, and prev/next pagination live on the dedicated page. The action-type suggestions include incident upload (`UPLOAD_BUNDLE`), civilian follow-up (`CIVILIAN_FOLLOWUP`), direct database insert (`DIRECT_DB_INSERT`), and Keycloak SPI action types. `DIRECT_DB_INSERT` rows are written by the `trg_detect_direct_fire_incident_insert` trigger (RP-20) when an INSERT into `wims.fire_incidents` occurs outside a trusted application session. **Keycloak SPI action types (WS-B, 2026-06-25):** `FAILED_LOGIN` (written when Keycloak fires `LOGIN_ERROR` or `USER_DISABLED_BY_BRUTE_FORCE` — true credential rejections invisible to the frontend) and `PASSWORD_RESET` (written when Keycloak fires `UPDATE_PASSWORD` or `RESET_PASSWORD_EMAIL` — Keycloak-native reset flow); both written by `POST /api/auth/keycloak-event` with `user_id=NULL` (no account-existence lookup) and `new_values.source="keycloak_spi"`. Filter `action_type=FAILED_LOGIN` or `action_type=PASSWORD_RESET` on `/admin/audit` to inspect these rows. |
...
**Suricata health states (GH #364):**

| State | Condition | Detail |
|---|---|---|
| `HEALTHY` | Recent threats detected in last 5 min | "alerting — recent threats detected" |
| `QUIET` | EVE log mtime < 60s, no recent threats, total > 0 | "healthy, no recent alerts" |
| `FRESH` | Total threats = 0 (fresh deployment) | "no threat data yet" |
