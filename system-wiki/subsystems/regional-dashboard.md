...
- Online incidents remain visible while offline, but cards/rows are disabled with `Go online to view` unless their detail payload is present in `cachedIncidents`. This avoids uncached detail navigations while preserving clickable pending-sync local incidents.
