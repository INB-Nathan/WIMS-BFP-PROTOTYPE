...
| F-20 | RP-25: Validator audit-log queries had no actor scoping — cross-user confidentiality leak | `build_audit_log_query()` (`services/regional_incidents/helpers.py`) had no forced current-actor scope; `actor_username` was an optional free-text ILIKE, so any `NATIONAL_VALIDATOR` leaving it blank saw every user's audit actions via both `validator.py:get_validator_audit_logs` (list) and `validator.py:export_validator_audit_logs` (CSV export — same leak). Fix: added a required `actor_user_id` param forcing `ivh.action_by_user_id = CAST(:actor_user_id AS uuid)`, mirroring the existing `encoder.py` self-scope pattern; `actor_username` remains only as an additional narrowing filter, ANDed on top of the forced scope, never a substitute for it. Also added a new self-scoped `GET /api/analytics/audit-logs` endpoint (`get_national_analyst`, `auth.py`) so `NATIONAL_ANALYST` has an own-actions audit view mirroring encoder/validator, instead of no endpoint at all. Regression coverage: `tests/test_validator_audit_log_query.py`. Branch `fix/525-role-scoped-audit-logs`, issue #525. | Issue #525 | Fixed in code; PR pending |
