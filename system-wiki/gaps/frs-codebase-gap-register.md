...
- **M14 (Public Submission — Zero-Trust Anonymous Reporting)**: CLOSED — FRS `#177`: `POST /api/v1/public/report` un-deprecated; writes to `wims.fire_incidents` with `encoder_id = NULL`, `verification_status = 'PENDING_VALIDATION'`; region resolved via inline `ST_Distance` nearest-centroid (no map.py dependency); Redis sliding-window rate limit 3/IP/hour with `Retry-After` header on 429; `src/backend/tests/test_public_submission.py` added.
