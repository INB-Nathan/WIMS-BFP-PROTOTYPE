...
sources: [raw/frs/frs-datacommitandimmutablestorage.md, raw/codebase/codebase-snapshot-2026-05-14.md, src/postgres-init/17_immutable_records.sql, src/postgres-init/72_partition_audit_trail.sql, src/backend/alembic/versions/0002_startup_schema_patches.py]
status: needs-review
---
...
- **[2026-07-21, CLOSED — GH #732] M4 audit-log append-only regression after partition replacement:** Raw FRS Module 4.b.iii requires audit logs to be immutable and append-only with no deletion. `17_immutable_records.sql` originally created `no_update_audit`/`no_delete_audit` RULES, and `72_partition_audit_trail.sql` replaced the protected plain table with a partitioned parent — but PostgreSQL rules cannot enforce on partitions, so the final schema had no DB-level UPDATE/DELETE control (application writes were RLS-limited, but RLS is bypassed by superuser-capable maintenance paths). Closed by `100_audit_trail_immutability.sql` (clean bootstrap) + Alembic revision `0031` (existing databases): parent-level BEFORE UPDATE/DELETE row triggers on `wims.system_audit_trails` raise an error (SQLSTATE 55000) on any prohibited mutation and are cloned onto current and future partitions. Semantics pinned in `tests/integration/test_audit_trail_immutability.py`: app role UPDATE/DELETE match zero rows; superuser UPDATE/DELETE raise; INSERT still allowed; future partitions receive the cloned triggers; `wims.incident_verification_history` remains independently append-only. Module 4 audit immutability is complete on the final schema.

- **[2026-07-17] #552 controlled routing — PARTIAL:** Issues #552 and #668 now have an internal-only OSRM service (`src/docker-compose.prod.yml`), a pinned Metro Manila dataset with SHA-256 verification (`src/osrm/metro-manila.env`, `scripts/provision-osrm-metro-manila.sh`), coordinate-safe backend routing (`src/backend/services/routing.py`), and a public receipt route map. Unit and Compose contract tests pass. Remaining gap: live production road routing requires an authorized dataset provisioning and VPS validation run; close fully only after `docs/operations/osrm-routing.md` is executed against provisioned data with health, synthetic-route, fallback, and privacy-log checks.
...
### KEYCLOAK-EMAIL-THEME-AUTO-ESCAPE (closed 2026-06-24)

- **Problem:** PR #453's 3 HTML email templates used `${displayName?html}`, `${link?html}`, and `${msg(...)?html}` (carried over from the v2.1 spec's "XSS fix" — explicit `<#assign displayName>` + `?html` on the output). The first live password-reset trigger (V1 tripwire) returned Keycloak's user-facing "Failed to send email, please try again later." `docker logs wims-keycloak` showed: `freemarker.core.ParseException: Syntax error in template "html/password-reset.ftl" in line 8, column 98: Using ?html (legacy escaping) is not allowed when auto-escaping is on with a markup output format (HTML), to avoid double-escaping mistakes`. **Root cause:** Keycloak 24 ships FreeMarker 2.3.32. Since FreeMarker 2.3.30, the default for HTML output format is `auto_esc=true` (auto-escape on). When auto-escape is on for a markup output format, the `?html` legacy escape builtin is rejected by the parser as a double-escape safeguard. This is a FreeMarker-level change, not Keycloak-level. The v2.1 self-review (17-row identifier-consistency table, every claim cited to the Keycloak 24 source) verified identifiers (`user.firstName`, `link`, `linkExpiration`, `linkExpirationFormatter`, message keys, template names) but did NOT verify the parseability of `?html` under FreeMarker 2.3.32 with auto-escape on. The v2 spec called `?html` "the XSS fix" — the fix was correct in spirit (escape untrusted user data) but the implementation (use the legacy `?html` builtin) is rejected by current FreeMarker.
- **Fix:** 7 `?html` call sites removed from 3 HTML templates (`password-reset.ftl` × 2, `email-verification.ftl` × 2, `executeActions.ftl` × 3). XSS protection now provided by FreeMarker's auto-escape, which is on for HTML output format and converts `& < > " '` to entities. The `<#assign displayName = (user.firstName?has_content)?then(user.firstName, user.username)>` pattern is preserved (the `?has_content` and `?then` builtins are not `?html` and parse fine; they provide null/empty-string safety independent of the output-time escape). The comment in `password-reset.ftl` lines 2-8 was rewritten to document the new pattern and the auto-escape reason. The 3 text templates (`text/*.ftl`) start with `<#ftl output_format="plainText">` and have auto-escape off; they do not use `?html` and require no change. Spec patched to v2.1.1 with a post-mortem section.
- **No scope creep:** zero application code changes; zero new tests; zero new SMTP creds; zero realm JSON changes; zero Dockerfile/compose changes; zero kcadm steps; the only VPS deploy step is `git pull --ff-only origin master && docker compose ... restart keycloak` to refresh the theme cache.
- **Spec:** `docs/superpowers/specs/2026-06-24-keycloak-email-theme-design.md` (v2.1.1 patch). Plan: `docs/superpowers/plans/2026-06-24-keycloak-email-theme.md` (v2 — unchanged, historical artifact). Closed by direct commit to `master` after PR #453 merge (small follow-up fix, no PR review required per operator instruction).
- **Lesson:** verify parseability, not just identifier consistency. A 17-row self-review table that cites every identifier to the Keycloak 24 source is necessary but not sufficient — running the templates through the live FreeMarker version (or `FreeMarkerException` checks) would have caught this. Also: FreeMarker auto-escape is the canonical XSS protection for HTML output format since 2.3.30. The operator-precedence XSS bug the v2.1 spec worried about (`${(user.firstName)!user.username?html}` — `?html` only escapes the fallback) does not apply when auto-escape is on; FreeMarker auto-escapes the resolved `${...}` value before any builtin runs.
- **Out of scope:** rewriting all 7 backend Jinja2 templates to use a consistent escape strategy; switching to FreeMarker's `?no_esc` (which would re-introduce the XSS); pre-compiling the templates to bytecode at build time.
...
- **Remaining open:** RP-05 (IVH UPDATE block — fixed in 67_repudiation_hardening.sql), RP-08+RP-18 server-side via Keycloak SPI (WS-B — merged PR #459), RP-20 (direct-insert detection, WS-C — merged PR #458), RP-23 (CSV export audit — fixed in branch AuditMoreGapsFix, PR pending).
- **[2026-06-28] RP-06 + RP-14 — NSD tamper detection + analytics export audit (WS-D, branch fix/ws-d-rp06-rp14-audit):**
  - **RP-06 (NSD recompute):** `verify_incident_hash_chain()` previously returned `"unverified"` for incidents with no IVH correction chain and never recomputed `data_hash` from current NSD — meaning a direct DB edit to `wims.incident_nonsensitive_details` was fully undetectable. Fix: provenance fetch now happens before the early return; `compute_incident_data_hash()` is called on every read; mismatch → `"tampered"` with `INTEGRITY_VIOLATION` audit row. Startup patch `_backfill_verified_data_hash()` in `main.py` populates `data_hash` for seeded VERIFIED incidents with NULL hash. Integration test: `tests/integration/test_rp06_nsd_tamper.py` (3 cases). **CLOSED.**
  - **RP-14 (analytics bulk export audit):** `POST /api/admin/privacy/export` already wrote `PII_EXPORT` to `system_audit_trails`, but CSV/PDF/Excel Celery exports (`/api/analytics/export/*`) wrote only to `analytics_export_log` — invisible to anomaly detectors. Fix: `_insert_export_log()` in `tasks/exports.py` now also calls `log_system_audit(..., "BULK_EXPORT", ...)` in the same transaction (non-fatal). **CLOSED.**
