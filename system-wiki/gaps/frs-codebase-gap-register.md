...
- **[OPEN — HIGH, 2026-07-10] M4 audit-log append-only regression after partition replacement:** Raw FRS Module 4.b.iii requires audit logs to be immutable and append-only with no deletion. `17_immutable_records.sql` creates `no_update_audit` and `no_delete_audit`, but the later ordered migration `72_partition_audit_trail.sql` renames and drops that protected table, creates a partitioned replacement, and restores RLS/indexes without recreating either rule. Alembic `0002_startup_schema_patches.py` restores the IVH update rule but not the two audit rules. The final fresh schema therefore lacks the intended DB-level UPDATE/DELETE rule protection (ordinary application writes remain constrained by RLS, but that is not equivalent to the FRS immutability control). Remediation: add a new Alembic revision and aligned post-72 clean-bootstrap SQL that restore final-parent protection, plus a regression test that attempts UPDATE and DELETE against the final partitioned table and verifies both are blocked. Do not mark Module 4 audit immutability complete until that final-schema test passes.
