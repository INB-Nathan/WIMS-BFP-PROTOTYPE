...
| `/` | `page.tsx` | Public civilian emergency report form. Includes the safety-first report flow, in-memory `PhotoUpload` selection during the details step, and post-submit online photo attachment with submitted-screen retry/error status. The Location step requires either coordinates or a nearby-landmark description before it can advance. Successful submissions expose the server-issued secure tracking URL (`/tracking/v2/{report_id}/{tracking_token}`) rather than a device-ID lookup path. |
...
- Shared public shell: `components/PublicHeader.tsx` is omitted on `/`, where `app/page.tsx` owns the immersive landing header. Other public/civilian routes use its sticky frosted header (`rgba(10, 10, 14, 0.82)` plus blur); the WIMS-BFP title is consistent with the landing page, and the `/report` route omits the redundant Report a Fire CTA and mobile FAB. `PublicThemeProvider` persists the public theme under `landing-theme` and defaults first-time visitors to the light scheme; landing map overlays and public maps consume the same theme values.
