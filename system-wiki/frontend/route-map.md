...
- Shared public shell: `components/PublicHeader.tsx` is omitted on `/`, where `app/page.tsx` owns the immersive landing header. Other public/civilian routes use its sticky frosted header (`rgba(10, 10, 14, 0.82)` plus blur); the WIMS-BFP title is consistent with the landing page, and the `/report` route omits the redundant Report a Fire CTA and mobile FAB.
