...
- Validation: `/dashboard/validator`, `/dashboard/validator/audit`, `/incidents/triage`. The validator dashboard uses quick status chips (All default, Pending indicator from stats), Today/Week/Month/Year/All Time and date-basis dropdowns, row click-to-view, and keeps audit access in the sidebar.
- Regional encoder: `/dashboard/regional` defaults its incident list to Today by Date Modified, renders Today incidents as richer cards, exposes date-range/date-basis dropdowns, and keeps activity-log access in the sidebar.
