...
- Validation: `/dashboard/validator`, `/dashboard/validator/audit`, `/incidents/triage`. The validator dashboard uses quick status chips (All default, Pending indicator from stats), Today/Week/Month/Year/All Time and date-basis dropdowns (`Date of Submission` default, `Date of Fire` optional), row click-to-view, compact status badges, and keeps audit access in the sidebar.
- Regional encoder: `/dashboard/regional` defaults its incident list to Today by Date Modified, renders Today incidents as richer cards, exposes date-range/date-basis dropdowns, and keeps activity-log access in the sidebar. `/dashboard/regional/incidents/[id]` uses a formal report-style read-only detail page with a compact header, non-status summary panel, desktop vertical dot section navigation, grouped section cards, compact affected-count cells, cleaner tables, and unchanged edit/delete/withdraw/submit/map behavior.
