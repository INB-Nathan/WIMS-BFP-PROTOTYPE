...
|| `/dashboard/validator/perimeter-draw` | `dashboard/validator/perimeter-draw/page.tsx` | Online NATIONAL_VALIDATOR/SYSTEM_ADMIN workspace. The validator queue exposes a Perimeter action for each VERIFIED incident, opening the workspace with its `incident_id`; the workspace loads stored jurisdiction metadata and saves a manually drawn GeoJSON polygon through the perimeter API, while client area remains preview-only. |
