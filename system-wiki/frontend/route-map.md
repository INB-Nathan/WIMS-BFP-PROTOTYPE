...
|| `/home` | `home/page.tsx` | Authenticated Operations tab label for all sidebar roles; route path unchanged. `/home` renders a split Operations console: a 70% operational map paired with a 30% operations/report panel on desktop and a map-first stacked layout on mobile. National Validators can create operations with optional linked civilian reports, manage linked reports from the selected-operation panel, and link/unlink civilian reports to active BFP operations. Regional Encoders, National Analysts, and System Administrators see linked report details read-only (no link/unlink controls). The `linkable-reports` search endpoint is `NATIONAL_VALIDATOR`-only. |
