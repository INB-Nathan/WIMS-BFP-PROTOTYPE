...
| 15 | `GET /top-n` | `top_n_route()` | Configurable top-N. Required: metric (incidents/response_time/casualties/damage_cost), dimension (barangay/fire_station/region/municipality). Region dimension returns ref-region names via `ref_regions` lookup with id fallback. |
