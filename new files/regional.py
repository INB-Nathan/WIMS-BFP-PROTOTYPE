...
def _parse_area_pair(val: Any) -> tuple[float, float]:
    """Parse a combined 'X / Y' area string into (floor_sqm, land_ha) floats.

    The AFOR form stores total floor area and total land area in a single cell
    as '100 / 0.3' (floor sqm / land ha). If no slash is present the whole
    value is treated as floor area and land area defaults to 0.
    """
    if val is None:
        return 0.0, 0.0
    s = str(val).strip()
...
class BfpXlsxParser:
    ...
    def parse(self) -> dict[str, Any]:
        ...
        # Always check row 48 (Structural) FIRST — a "/" mark there must win over
        # unmarked rows 49/50 even when those rows carry "N/A" dropdown text.
        def _na(v: Any) -> bool:
            """True when v is missing, blank, or the literal placeholder 'N/A'."""
        ...
        if self._is_marked_on_row(48):
            ...
            # No checkbox marked — infer from first non-null, non-N/A dropdown value
        ...
        # Extent of Damage — use exact spreadsheet C-column labels
        _extent_label_map = {
        ...
        extent_dval: Any = None  # D value from the SELECTED extent row (may be "X / Y" area pair)
        for _ext_row, _ext_label in _extent_label_map.items():
            if self._is_marked_on_row(_ext_row):
                extent = _ext_label
                _raw_d = self.get(f"D{_ext_row}")
                extent_dval = None if (_raw_d is None or str(_raw_d).strip().upper() == "N/A") else _raw_d
                break
        else:
            # No checkbox found — fallback to first real (non-N/A) D value across all extent rows
            for _ext_row in _extent_label_map:
                _raw_d = self.get(f"D{_ext_row}")
                if _raw_d is not None and str(_raw_d).strip().upper() not in ("", "N/A"):
                    extent_dval = _raw_d
                    break
        ...
        return {
            ...
            "extent_total_land_area_hectares": extent_dval,  # same cell — _parse_area_pair will split "X / Y"
...
def parse_afor_report_data(data: dict, region_id: int) -> AforParsedRow:
    ...
    time_engine_dispatched = _safe_dt(
        data.get("time_dispatched") or data.get("time_engine_dispatched")
    )
    time_arrived_at_scene = _safe_dt(
        data.get("time_arrived") or data.get("time_arrived_at_scene")
    )
    time_returned_to_base = _safe_dt(
        data.get("time_returned") or data.get("time_returned_to_base")
    )
    receiver_name_val = (
        data.get("receiver") or data.get("receiver_name") or ""
    )

    # ── Section B: classification aliases ────────────────────────────────────
    classification_of_involved = (
        data.get("classification") or data.get("classification_of_involved") or ""
    )
    type_of_involved_general_category = (
        data.get("category") or data.get("type_of_involved_general_category") or ""
    )
    general_description_of_involved = (
        data.get("description") or data.get("general_description_of_involved") or ""
    )
    stage_of_fire_upon_arrival = (
        data.get("stage") or data.get("stage_of_fire_upon_arrival") or ""
    )

    # ── Section B: parse "100 / 0.3" combined area cell ─────────────────────
    ...
    incident_nonsensitive_details = {
        ...
        # Section A — response details (available in preview & form; subset stored in DB)
        ...
        "distance_to_fire_scene_km": _safe_float(
            data.get("distance_km")
            if data.get("distance_km") is not None
            else data.get("distance_to_fire_scene_km")
        ),
        "receiver_name": receiver_name_val,
        # Section B — nature & classification aliases (stored in general_category/sub_category)
        ...
        "resources_deployed": {
            # Response Vehicles — keys match spreadsheet row labels verbatim
            "bfp_fire_trucks": _safe_int(
        ...
        # Location text fields — kept in ns for import-preview form pre-fill.
        # These do NOT have DB columns in incident_nonsensitive_details so they
        # are ignored by the INSERT but travel in the preview payload.
    ...
    # Re-build caller_info string at the top-level so the commit handler's PII
    # encryption path can find it (it reads row_data["caller_info"], not the
    # already-split sensitive_details fields).
    _caller_info_str = ""
    if c_name and c_num:
        _caller_info_str = f"{c_name} / {c_num}"
    elif c_name:
        _caller_info_str = c_name
    elif c_num:
        _caller_info_str = c_num

    mapped = {
        "region_id": region_id,
        "caller_info": _caller_info_str,  # ← top-level for commit handler
...
async def commit_afor_import(
    ...
    for idx, row_data in enumerate(body.rows):
        ...
        #
        # Primary path: caller_info at top-level row_data (set by parse_afor_report_data).
        # Fallback: read from incident_sensitive_details if top-level key is absent
        # (e.g. older payloads or manual-entry bundles that split the fields already).
        ...
        if ci:
            ...
            # Fallback: caller already split in incident_sensitive_details
            _sd_pre = row_data.get("incident_sensitive_details") or {}
            caller_name_row = str(_sd_pre.get("caller_name") or "").strip()
            caller_number_row = str(_sd_pre.get("caller_number") or "").strip()
...
def get_regional_incident_detail(
    ...
    # Fetch responding unit (dispatch/arrival/return times + engine number)
    ru_row = db.execute(
        text(
            "SELECT station_name, engine_number, responder_type, dispatch_dt, arrival_dt, return_dt "
            "FROM wims.responding_units WHERE incident_id = :iid ORDER BY response_id ASC LIMIT 1"
        ),
        {"iid": incident_id},
    ).fetchone()
    ...
    responding_unit_dict: dict = {}
    if ru_row is not None:
        dispatch_dt = ru_row[3]
        arrival_dt = ru_row[4]
        return_dt = ru_row[5]
        responding_unit_dict = {
            "station_name": ru_row[0],
            "engine_number": ru_row[1],
            "responder_type": ru_row[2],
            "dispatch_dt": dispatch_dt.isoformat() if dispatch_dt else None,
            "arrival_dt": arrival_dt.isoformat() if arrival_dt else None,
            "return_dt": return_dt.isoformat() if return_dt else None,
        }
    ...
    return {
        ...
        "sensitive": sd_dict,
        "responding_unit": responding_unit_dict,
    }
