...
21. **Keycloak 26 bootstrap admin is temporary (cannot use kcadm.sh).** Bootstrap admin accounts created via `KC_BOOTSTRAP_ADMIN_USERNAME`/`KC_BOOTSTRAP_ADMIN_PASSWORD` are intentionally temporary (`is_temporary_admin = true`) and cannot authenticate via `grant_type=password`. Running `kcadm.sh config credentials` fails with `"Account is not fully set up [invalid_grant]"`. To use kcadm.sh for scripted realm configuration, create a permanent admin user through the Admin Console or REST API after bootstrap, then use that user's credentials with kcadm.sh. See `src/keycloak/bootstrap/bootstrap-master-realm.sh` and `https://www.keycloak.org/server/bootstrap-admin-recovery`.
