...
- **Analyst export — pre-existing SQL contract bug in `get_export_rows()` (discovered during #112):** `fact_cols` declared nine columns (`civilian_injured`, `civilian_deaths`, `firefighter_injured`, `firefighter_deaths`, `total_response_time_minutes`, `estimated_damage_php`, `fire_station_name`, `municipality_name`, `province_name`) as belonging to `wims.analytics_incident_facts`, but per `\d wims.analytics_incident_facts` only `incident_id, region_id, location, notification_dt, notification_date, alarm_level, general_category, synced_at` exist on that table. Selecting any default-set column except the five truly on facts produced `psycopg2.errors.UndefinedColumn`; the existing pytest suite mocked `get_analyst_export_rows` so the bug never surfaced in CI. Fix: `fact_cols` trimmed to the five columns actually present; misplaced columns now resolve via `nd.*` from `wims.incident_nonsensitive_details`; `municipality_name` now resolves via `rc.city_name AS municipality_name` joining `wims.ref_cities rc ON rc.city_id = nd.city_id`; `province_name` now resolves via `rp.province_name` joining `wims.ref_provinces rp ON rp.province_id = rc.province_id`. Both new joins are conditional — added only when the corresponding column is selected. (`src/backend/services/analytics_read_model.py::get_export_rows`)
- **Known related issue (out of scope, follow-up ticket recommended):** The same incorrect column-to-table assumption exists at `src/backend/api/routes/incidents.py:835-836` in the National Analyst incident detail endpoint, which selects `aif.province_name` and `aif.municipality_name` from `wims.analytics_incident_facts`. This endpoint is unaffected by the present PR but should be remediated using the same `nd.city_id → ref_cities → ref_provinces` pattern.
