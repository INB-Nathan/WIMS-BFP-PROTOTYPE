...
### Security
- **Public rate limits anchored to TCP socket IP (PR #446, gap #14).** All per-IP rate limits (civilian reports, public DMZ incidents, follow-ups, notify registrations) are now keyed on `$realip_remote_addr` (the actual TCP socket IP after nginx's `real_ip_module` rewrites it), not on the client-controlled `X-Forwarded-For` header. The `set_real_ip_from` trust range in all 3 nginx configs is narrowed from `172.16.0.0/12` (1,048,576 addrs) to `172.18.0.0/16` (the actual Docker Compose v2 default custom bridge). Audit log `ip_address` and `ip_hash` columns now mirror the rate-limit key (the trusted socket IP), not the XFF-controlled value. The DB rate limit on `POST /api/civilian/reports` is lowered from 5/hr to 3/hr to align with `/api/v1/public/report` (gap #14 anchor change). The audit log regression is closed by extracting `trusted_client_ip(request)` in `utils/audit.py` and using it at the 6 call sites that previously read `X-Forwarded-For` (3× civilian.py, 1× public_dmz.py rate limiter, 2× audit). The TOCTOU race in the DB rate limit is closed by taking `pg_advisory_xact_lock` before the `COUNT(*)` query. `Retry-After` is added to `Access-Control-Expose-Headers` in all 8 public/civilian CORS blocks (3 nginx configs) so cross-origin browser callers can read it. The `"unknown"` fallback for missing `X-Real-IP` / `request.client` is now per-request (`"unknown:<x-request-id>:<uuid4>"`), not a single global bucket. (`src/backend/utils/audit.py`, `src/backend/utils/rate_limit.py`, `src/backend/api/routes/civilian.py`, `src/backend/api/routes/public_dmz.py`, `src/nginx/nginx.conf`, `src/nginx/nginx.ci.conf`, `src/nginx/nginx.local.conf`)
...
- **Documentation cleanup — removed stale system-wiki references and pruned docs/:** Updated `CLAUDE.md`, `infra/AGENTS.md`, `docs/AGENTS.md`, and 5 AGENTS.md files to replace stale `system-wiki/` references with code-level sources of truth. Fixed `docs/agents/gotchas.md` item 3 (FRS module citation guidance). Fixed `docs/deployment-inventory.md` and `src/nginx/bot-blocker/README.md` system-wiki references. Removed ~90 stale documentation artifacts including `docs/handoffs/`, `docs/reviews/`, `docs/superpowers/`, `docs/implementations/`, `docs/agents/plans/`, standalone stale docs, stale API references, outdated schema PDFs, and the 500-page user manual. Kept compliance/regulatory docs, operations runbooks, security specs, and operational guidance per "code as source of truth" philosophy. (`AGENTS.md`, `CLAUDE.md`, `infra/AGENTS.md`, `docs/AGENTS.md`, `src/AGENTS.md`, `src/backend/AGENTS.md`, `src/frontend/AGENTS.md`, `docs/agents/gotchas.md`, `docs/deployment-inventory.md`, `src/nginx/bot-blocker/README.md`)
...
- **Encoder region assignment:** Added the NCR encoder to region assignment in `05_validator_workflow.sql` migration. REGIONAL_ENCODER users require assigned_region_id for authorization checks; missing value caused client-side dashboard redirect loops during development.
