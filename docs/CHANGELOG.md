...
- **Frontend session API client:** `fetchUserSessions(userId)` and `terminateUserSessions(userId, sessionId)` added to `src/frontend/src/lib/api.ts` for SYSTEM_ADMIN session management flows.
...
- **`accessTokenLifespan`:** Keycloak access tokens remain short-lived at 300 s (5 min). The prior proposal to increase this to 3600 s (1 h) was rejected; the accepted fix is proactive refresh via `POST /api/auth/refresh` while the Keycloak/browser session remains valid. (`src/keycloak/bfp-realm.json`, `src/frontend/src/app/api/auth/refresh/route.ts`, `src/frontend/src/context/AuthContext.tsx`)
- **Proactive JWT refresh:** AuthProvider now refreshes the HttpOnly `access_token` cookie every 4 minutes, retries session loading once after a 401 by refreshing, and refreshes when the tab regains focus/visibility. This fixes premature `JWT Signature has expired` errors without broadening access-token lifetime. (`src/frontend/src/context/AuthContext.tsx`)
- **Cookie lifetime alignment:** `access_token` cookie `maxAge` is now 300 s to match the Keycloak access-token lifespan; `refresh_token` cookie `maxAge` is 28800 s to match the 8 h SSO session max. (`src/frontend/src/app/api/auth/sync/route.ts`, `src/frontend/src/app/api/auth/refresh/route.ts`)
...
### Fixed
- **Analyst export — phantom `barangay_name` column removed (#111):** `barangay_name` no longer appears in the export column picker (`ExportPreviewModal.tsx`) or the National Analyst incident table (`AnalystIncidentList.tsx`). The backend analytics read model never populated this column — see explanatory comment in `src/backend/api/routes/incidents.py:837`. Also removed from `AnalystIncidentListItem` and `AnalystListSortField` in `src/frontend/src/lib/api.ts`, and from the orphaned summary/detail panels (`AnalystIncidentList.tsx` drawer and `src/frontend/src/app/dashboard/analyst/incidents/[id]/page.tsx`).
- **Analyst export — `region_name` replaces raw `region_id` (#112):** Export rows previously contained `region_id` as a raw integer (e.g. `4`). The picker now offers a human-readable `region_name` (e.g. `Region IV-B (CALABARZON)`) selected by default; `region_id` remains available as a separate column. Implementation: added `region_name` to `ALLOWED_EXPORT_COLUMNS` in `src/backend/tasks/exports.py`, and `get_export_rows()` in `src/backend/services/analytics_read_model.py` now conditionally joins `wims.ref_regions rr` and selects `rr.region_name`.
- **Analyst export — curated default column selection (#113):** Default selection changed from `ALL_COLUMNS.slice(0, 6)` (which included the phantom barangay column and raw `region_id`) to an explicit 9-column high-signal set: `incident_id, notification_dt, region_name, province_name, municipality_name, general_category, alarm_level, estimated_damage_php, total_response_time_minutes`. Backend `DEFAULT_EXPORT_COLUMNS` updated to match. (`src/frontend/src/components/analytics/ExportPreviewModal.tsx`, `src/backend/tasks/exports.py`)
- **Analyst module — systemic wrong-table column references (discovered during #112 testing, bundled per bug-discovery convention):** `wims.analytics_incident_facts` has only 8 columns per the live schema (`incident_id, region_id, location, notification_dt, notification_date, alarm_level, general_category, synced_at`), but 17 functions across `services/analytics_read_model.py` and `api/routes/incidents.py` issued queries that referenced 9 phantom columns (`province_name`, `municipality_name`, `civilian_injured/deaths`, `firefighter_injured/deaths`, `total_response_time_minutes`, `estimated_damage_php`, `fire_station_name`). Three sync functions also attempted to INSERT into the same 9 nonexistent columns, leaving the cache table permanently empty (0 rows despite verified incidents existing). The original denormalization migration these references assume was never written; the live schema only carries the 8 columns. User-visible result before the fix: `/dashboard/analyst/response-time`, `/top-n`, `/comparative`, and `/incidents/<id>` returned 500; `/heatmap` and `/trends` returned 500 when location/casualty/damage filters were applied. The existing pytest suite mocked the data layer (`get_analyst_export_rows`) so none of this surfaced in CI. Fix:
  - **Read path:** every `a.<bad-col>` / `aif.<bad-col>` reference rewritten to read from the actual source-of-truth tables — `nd.<col>` from `incident_nonsensitive_details` for damage/casualty/response/station, `rc.city_name` from `ref_cities` for `municipality_name` (joined via `nd.city_id`), and `rp.province_name` from `ref_provinces` for `province_name` (joined via `rc.province_id`). The added JOINs are conditional in `get_heatmap_points`, `get_trends`, `count_in_range`, `get_type_distribution`, `get_response_time_by_region`, `get_compare_regions`, and `get_top_n` (emitted only when filters / dimensions actually need them); the analyst list and detail endpoints in `incidents.py` always join because they always SELECT location names. `VALID_TOP_N_DIMENSIONS` re-mapped: `fire_station` → `nd.fire_station_name`, `municipality` → `rc.city_name`. `get_filter_options` rewritten to resolve province/municipality from the reference tables joined through `nd.city_id`. `_append_common_filters` updated so all callers can pass casualty/damage/location filters through the same helper.
  - **Sync path:** `sync_incident_to_analytics`, `sync_incidents_batch`, and `backfill_analytics_facts` trimmed to INSERT only the 7 columns that actually exist on `analytics_incident_facts`. The cache now populates on first verified-incident sync, and the `sync_status` indicator on the analyst detail page becomes meaningful for the first time (verified via `backfill_analytics_facts` → 4 rows from 4 verified incidents in dev). Also fixed a latent SQLAlchemy bug where `:rows::jsonb` (named-param marker `:rows` colliding with the `::jsonb` cast operator) raised `syntax error at or near ":"`; rewritten as `CAST(:rows AS jsonb)`.
  - **Test contract:** `tests/test_analyst_incidents_sql_contract.py::test_analyst_list_query_uses_current_location_columns` was previously asserting the buggy `aif.<col>` references existed in the analyst list query — flipped to assert the correct `rc.city_name` / `rp.province_name` refs. `test_sync_incident_populates_casualty_columns` and `test_sync_incident_populates_station` in `tests/integration/test_analyst_dashboard_queue.py` were asserting the sync wrote casualty/station columns into facts — replaced with `test_sync_incident_does_not_write_nonexistent_columns` and `test_sync_incident_writes_only_real_facts_columns` which reflect the correct contract.
  - **Verification:** live-DB smoke tests via `docker exec wims-backend python -c ...` for all 17 fixed functions executed without `UndefinedColumn` errors; `pytest tests/test_analyst_export.py tests/test_analyst_incidents_sql_contract.py tests/integration/test_analyst_dashboard_queue.py::TestPhase1Foundation` 26/26 passed; full backend suite shows net −2 failures vs pre-PR baseline (the 2 sync contract tests now correctly assert the new behavior; all remaining failures are pre-existing schema mismatches in regional-encoder / narrative / monitoring fixtures and are unrelated to the analyst module).
  - **Files touched:** `src/backend/services/analytics_read_model.py` (11 functions including the 3 sync paths), `src/backend/api/routes/incidents.py` (3 functions: `_append_analyst_casualty_filter`, `get_analyst_incident_list`, `get_analyst_incident_detail`), `src/backend/tests/test_analyst_incidents_sql_contract.py`, `src/backend/tests/integration/test_analyst_dashboard_queue.py`.
